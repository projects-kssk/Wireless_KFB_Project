...
export async function ensureEspPresent(): Promise<void> {
  const present = await isEspPresent().catch(() => false);
  if (!present) throw new Error("serial-not-present");
}

export async function writeToStation(payload: string, mac: string): Promise<void> {
  const target = mac.toUpperCase();
  if (!MAC_RE.test(target)) throw new Error('invalid-mac');
  const { port } = getEspLineStream() as any; // { parser, port }
  await new Promise<void>((resolve, reject) =>
    port.write(`${payload} ${target}\n`, (err: any) => (err ? reject(err) : resolve())),
  );
}

type WaitMode = "none" | "echo" | "result";

export function waitForEchoOrResult(opts: {
  signal: AbortSignal;
  mac: string;          // normalized uppercase
  payload: string;
  mode: WaitMode;       // "echo" waits for ECHO OK; "result" waits for SUCCESS/FAILURE
  timeoutMs?: number;   // default 15000
}): Promise<{ event: "echo" | "result"; reply: string; lines: string[] }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const isResultReply = (s: string) =>
      /\b(SUCCESS|FAILURE)\b/.test(s.toUpperCase());
