...
function armEsp(): EspLineStream {
  ...
  const parser = port.pipe(normalizer).pipe(new ReadlineParser({ delimiter: "\n" }));
...
export async function espHealth(opts?: {
  ...
  if (probe === "never") {
    return {
      present,
      ok: true,
      raw: act.ageMs == null ? "present (no activity yet)" : `passive ${ageMs}ms`,
      ageMs: act.ageMs ?? undefined,
      lastLine: act.lastLine,
    };
  }
  ...
  if (!tmpl) {
    return {
      present,
      ok: true,
      raw: act.ageMs == null ? "present (probe disabled)" : `passive ${ageMs}ms`,
      ageMs: act.ageMs ?? undefined,
      lastLine: act.lastLine,
    };
  }
...
/* ============================================================================
   Scanner fleet (multi-port singleton with backoff)
   ========================================================================== */
...
const GG2 = globalThis as any;
if (!GG2.__scanners) GG2.__scanners = new Map<string, Runtime>();
const scanners: Map<string, Runtime> = GG2.__scanners;
...
const resetCooldown = (r: RetryState) => {
  r.retryMs = 2000;
  r.nextAttemptAt = 0;
};
...
function looksLikeSamePath(candidate: string, wanted: string) {
  ...
  if (m) {
    const idx = m[1];
    return (
      candidate.toLowerCase().endsWith(tail) ||
...
function sanitizePayload(raw: unknown, path: string): string | null {
  let code = String(raw ?? "");
  code = code.replace(/[\r\n\0]+/g, "");
  if ((process.env.SCANNER_PRINTABLE_ONLY ?? "1") !== "0") code = code.replace(/[^\x20-\x7E]/g, "");
  ...
  if (acceptAny) return code;

  // Default: accept MAC-like frames only
  const m = code.match(/([0-9A-F]{2}(?:[:\-]?[0-9A-F]{2}){5})/i);
  if (!m) return null;
  const out = m[1].replace(/[^0-9A-F]/gi, "").match(/.{2}/g)!.join(":").toUpperCase();
  try {
...
function attachScannerHandlers(
  ...
  parser.on("data", (raw: unknown) => {
    const out = sanitizePayload(raw, path);
    if (!out) return;
    broadcast({ type: "scan", code: out, path });
  });

  // Optional ASCII-without-newline fallback using idle timer (single implementation)
  if ((process.env.SCANNER_ALLOW_ASCII_NO_NL ?? "0") === "1") {
    ...
    const flushAscii = () => {
      ...
      asciiBuf = "";
      if (!payload) return;
      const out = sanitizePayload(payload, path);
      if (!out) return;
      try {
        if ((process.env.SCAN_LOG ?? "0") === "1")
          LOG.tag("scanner").info("scan(ascii-no-nl)", { code: out, path });
      } catch {}
      broadcast({ type: "scan", code: out, path });
    };
    ...
    try {
      (port as any).on("data", asciiHandler);
      (port as any).once("close", () => {
        try {
          (port as any).off?.("data", asciiHandler);
        } catch {}
        if (t) clearTimeout(t);
        asciiBuf = "";
      });
...
export async function ensureScannerForPath(path: string, baudRate = 115200): Promise<void> {
  ...
  if (!ALLOW_USB_SCANNER && /\/ttyUSB\d+$/i.test(path)) {
    const esp = process.env.ESP_TTY ?? process.env.ESP_TTY_PATH ?? "";
    const LOG_SKIPS = (process.env.LOG_SCANNER_SKIPS ?? "0") === "1";
    if (LOG_SKIPS) {
      if (esp && path === esp) LOG.tag("scanner").info(`skip scanner on ESP path ${path}`);
  ...
  state.starting = new Promise<void>((resolve, reject) => {
    ...
    port.on("open", () => {
      ...
      try {
        LOG.tag("monitor").info(`SCANNER OPEN ${path}`);
      } catch {}
      assertControlLines();
    });
...
export async function ensureScanners(pathsInput?: string | string[], baudRate = 115200): Promise<void> {
  ...
  try {
    ...
    // Add ACM devices, including /by-id/ symlinks
    const acmPaths = list
      .map((d) => d.path)
    ...
    // Prefer Honeywell vendor if present
    try {
      const honey = list
    ...
    // Optionally include ttyUSB devices
...
export function getScannerStatus() {
  const obj: Record<
    string,
    { open: boolean; inCooldown: boolean; nextAttemptAt: number; lastError: string | null }
  > = {};
...
export function considerDevicesForScanner(
  ...
  const paths = [...pathsInput.split(",").map((s) => s.trim()).filter(Boolean), ...(extra ? [extra] : [])];
...
/* ============================================================================
   Process cleanup (dev/hot-reload safe)
   ========================================================================== */
