...
const DEFAULT_API_MODE = process.env.NEXT_PUBLIC_KROSY_ONLINE === "true" ? "online" : "offline";
...
const ENDPOINT_ONLINE = process.env.NEXT_PUBLIC_KROSY_URL_ONLINE ?? "/api/krosy";
const ENDPOINT_OFFLINE = process.env.NEXT_PUBLIC_KROSY_URL_OFFLINE ?? "/api/krosy-offline";
// Checkpoint endpoints
const ENDPOINT_CHECKPOINT_ONLINE = process.env.NEXT_PUBLIC_KROSY_URL_CHECKPOINT_ONLINE ??
    "/api/krosy/checkpoint";
const ENDPOINT_CHECKPOINT_OFFLINE = process.env.NEXT_PUBLIC_KROSY_URL_CHECKPOINT_OFFLINE ??
    "/api/krosy-offline/checkpoint";
// Identity bootstrap (GET returns hostname/ip/mac from the offline checkpoint route)
const IDENTITY_ENDPOINT = process.env.NEXT_PUBLIC_KROSY_IDENTITY_URL ??
    "/api/krosy-offline/checkpoint";
...
function formatXml(xml) {
    try {
        ...
        return out
            ...
            .map((ln) => {
            let indent = 0;
            if (ln.match(/^<\/\w/) || ln.match(/^<\w[^>]*\/>/))
                indent = -1;
            const line = `${"  ".repeat(Math.max(pad + indent, 0))}${ln}`;
            if (ln.match(/^<\w[^>]*[^/]>/) && !ln.startsWith("<?"))
                pad += 1;
            if (ln.match(/^<\/\w/))
...
const isCompleteKrosy = (xml) => /^\s*(?:<\?xml[^>]*\?>\s*)?<krosy[\s>][\s\S]*<\/krosy>\s*$/i.test(xml);
/* ===== component ===== */
export default function KrosyPage() {
    ...
    const [respBody, setRespBody] = useState("Run the test to see response payload…");
    const [xmlPreview, setXmlPreview] = useState("");
    const termRef = useRef(null);
    // checkpoint gate
    const [hasWorkingData, setHasWorkingData] = useState(false);
    const [workingDataXml, setWorkingDataXml] = useState(null);
    ...
    const endpointCheckpoint = apiMode === "offline" ? ENDPOINT_CHECKPOINT_OFFLINE : ENDPOINT_CHECKPOINT_ONLINE;
    const accept = useMemo(() => (mode === "json" ? "application/json" : "application/xml"), [mode]);
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                if (!r.ok)
                    throw new Error(`bootstrap GET ${r.status}`);
                    ...
                    if (ip && IP_ONLINE && ip === IP_ONLINE)
                        ...
                        for (const u of urls) {
                            try {
                                ...
                                if (ip2 && IP_ONLINE && ip2 === IP_ONLINE) {
                                    setApiMode('online');
                                    break;
                                }
                                if (ip2 && IP_OFFLINE && ip2 === IP_OFFLINE) {
                                    setApiMode('offline');
                                    break;
                                }
                            }
    ...
    const extractXmlFromResponse = (ct, payload) => {
        if ((ct || "").includes("json")) {
            const j = payload;
            return String(j.responseXmlRaw ||
                j.responseXml ||
    ...
    const refetchFullXmlIfNeeded = useCallback(async (payload) => {
        if (apiMode !== "online")
            return null;
        try {
    ...
    const run = useCallback(async () => {
        if (inFlightRef.current || busyAny)
            return;
        ...
        try {
            ...
            if (ct.includes("json")) {
                ...
                const srvIsComplete = typeof j.isComplete === "boolean" ? j.isComplete : isCompleteKrosy(xml);
                const srvIsPreview = typeof j.isPreview === "boolean" ? j.isPreview : (!srvIsComplete && !!xml);
    ...
    const canCheckpoint = useMemo(() => hasWorkingData && !!workingDataXml && isCompleteKrosy(workingDataXml) && checkpointEligible, [hasWorkingData, workingDataXml, checkpointEligible]);
    const endpointLabel = apiMode === "offline" ? ENDPOINT_OFFLINE : ENDPOINT_ONLINE;
    const checkpointLabel = apiMode === "offline" ? ENDPOINT_CHECKPOINT_OFFLINE : ENDPOINT_CHECKPOINT_ONLINE;
    return (<div className="mx-auto max-w-6xl px-4 sm:px-6" style={{ paddingTop: "max(env(safe-area-inset-top),1rem)", paddingBottom: "max(env(safe-area-inset-bottom),1rem)" }}>
      {/* Header */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <m.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="rounded-3xl border border-black/5 bg-white/95 dark:bg-gray-900/70 backdrop-blur-xl shadow-[0_8px_24px_rgba(0,0,0,0.08)] p-5">
          ...
          <div className="mb-4 flex items-center justify-between gap-3 flex-wrap">
            <Segmented value={mode} options={[{ id: "json", label: "JSON" }, { id: "xml", label: "XML" }]} onChange={(v) => setMode(v)}/>
            <Segmented value={apiMode} options={[{ id: "online", label: "ONLINE" }, { id: "offline", label: "OFFLINE" }]} onChange={(v) => setApiMode(v)} intentById={{ online: "success", offline: "danger" }}/>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3 pt-2 flex-wrap">
              <m.button onClick={run} whileHover={{ y: -1 }} whileTap={{ scale: 0.98 }} disabled={busyAny || canCheckpoint} className="inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 shadow" aria-label="Send request">
                <Spinner visible={busyReq}/>
                {busyReq ? "Sending…" : "Send"}
              </m.button>

              <m.button onClick={runCheckpoint} whileHover={{ y: -1 }} whileTap={{ scale: 0.98 }} disabled={busyAny || !canCheckpoint} className="inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 shadow" aria-label="Send checkpoint from response XML" title={canCheckpoint ? "Build and send workingResult from workingData XML" : "Run request and ensure full XML"}>
      ...
      <m.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: 0.1 }} className="mt-6 rounded-3xl border border-black/5 bg-white/95 dark:bg-gray-900/70 backdrop-blur-xl shadow-[0_8px_24px_rgba(0,0,0,0.08)]">
        <div className="flex items-center justify-between px-4 py-2 text-xs text-gray-600 dark:text-gray-300 border-b border-black/5">
          <div className="flex items-center gap-2">
            ...
            {xmlPreview && mode === "json" ? (<div className="ml-3 inline-flex rounded-xl bg-gray-100 dark:bg-gray-800 p-0.5">
                {["body", "xmlPreview"].map((t) => (<button key={t} onClick={() => setTab(t)} className={[
...
/* UI bits */
function Segmented({ value, options, onChange, intentById }) {
    return (<div className="inline-flex rounded-2xl bg-gray-100 dark:bg-gray-800 p-1 shadow-inner" role="tablist" aria-label="segmented control">
      {options.map((opt) => {
            const active = value === opt.id;
            const intent = intentById?.[opt.id] ?? "neutral";
            const activeCls = intent === "success"
            ...
            return (<m.button key={opt.id} whileTap={{ scale: 0.98 }} onClick={() => onChange(opt.id)} className={["px-3.5 py-2 text-sm font-medium rounded-xl min-w-[84px] transition", active ? `${activeCls} shadow` : "text-gray-600 dark:text-gray-300"].join(" ")} aria-pressed={active} role="tab">
            {opt.label}
          </m.button>);
        })}
    </div>);
}
function ConnectivityPill({ apiMode, endpoint }) {
    const online = apiMode === "online";
    return (<div className={["inline-flex items-center gap-2 rounded-2xl px-2.5 py-1.5 text-xs font-medium", online ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"].join(" ")} title={endpoint}>
...
function Input({ onValueChange, className = "", disabled, ...rest }) {
    return (<input {...rest} disabled={disabled} onChange={(e) => onValueChange?.(e.target.value)} className={[
...
function Spinner({ visible }) {
    return (<AnimatePresence initial={false}>
      {visible ? (<m.span className="inline-block h-4 w-4 rounded-full border-2 border-white/70 border-t-transparent" style={{ borderRightColor: "transparent" }} animate={{ rotate: 360 }} transition={{ repeat: Infinity, ease: "linear", duration: 0.8 }}/>) : null}
    </AnimatePresence>);
}
function StatusPill({ status, http, duration }) {
    const map = {
    ...
    const s = map[status];
    return (<m.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className={`inline-flex items-center gap-2 rounded-2xl px-3 py-1.5 text-sm ${s.cls}`}>
      <span>{s.label}</span>
