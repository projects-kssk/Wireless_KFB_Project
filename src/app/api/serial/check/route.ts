...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (typeof helper.sendAndReceive !== 'function') throw new Error('sendAndReceive not found')
    sendAndReceive = helper.sendAndReceive as (cmd: string) => Promise<string>
  } catch (err) {
  ...
  try {
    ...
    if (text === 'SUCCESS' || text === 'OK') return NextResponse.json({ failures: [] })
