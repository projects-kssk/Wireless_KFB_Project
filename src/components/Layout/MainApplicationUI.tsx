...
const MainApplicationUI: React.FC = () => {
  ...
  const clearSoftIdleTimer = useCallback(() => {
    if (softIdleTimerRef.current != null) {
      try {
        clearTimeout(softIdleTimerRef.current);
      } catch {}
  ...
  const showScanResult = (text: string, kind: "info" | "error" = "info", ms: number = 1000) => {
    try {
      if (scanResultTimerRef.current) clearTimeout(scanResultTimerRef.current);
    } catch {}
    // Show headline result and ensure we exit SCANNING state
    setIsScanning(false);
    setShowScanUi(false);
    ...
    // Also schedule a soft idle so MAC/headline return to "Please Scan Barcode" even if other timers fail
    try { scheduleSoftIdle(ms); } catch {}
  };

  // Scanning watchdog: never get stuck in SCANNING…
  useEffect(() => {
  ...
  const showHeadlineResult = useCallback(
    (text: string, kind: "info" | "error" = "info", ms = 2000) => {
      showScanResult(text, kind, ms);
      scheduleSoftIdle(ms);
    },
    [scheduleSoftIdle]
  );
  ...
  const showOverlay = (kind: OverlayKind, code: string) => {
    if (kind === "error") {
      // No error overlays. Use headline result instead.
      setErrorMsg(code || "Error");
      return;
    }
    if (kind === "scanning") return; // Never show scanning overlays
  ...
  const startScanOverlayTimeout = (ms = Math.max(1000, Number(process.env.NEXT_PUBLIC_SCAN_OVERLAY_MS ?? "3000"))) => {
    ...
    // Scanning overlay disabled. Keep API for compatibility.
              ...
              try {
                try {
                  ...
                  if (r.ok) {
                    ...
                    if (ids.length) {
                      try {
                        console.log("[FLOW][CHECKPOINT] STOP path: sending for all ids", {
                          mac: target,
                          count: ids.length,
                        });
                      } catch {}
  ...
  useEffect(() => {
    try {
        ...
        if (redisDropTimerRef.current == null) {
          ...
          redisDropTimerRef.current = window.setTimeout(() => {
            ...
            if (!redisReadyRef.current) {
              const ms = lastRedisDropAtRef.current ? Date.now() - lastRedisDropAtRef.current : undefined;
      ...
      if (prev === false && ready === true) {
        ...
        const msDown = lastRedisDropAtRef.current ? Date.now() - lastRedisDropAtRef.current : undefined;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const nm = u.names && typeof u.names === "object" ? Object.keys(u.names).length : 0;
      ...
      const actIds =
        lastActiveIdsRef.current && lastActiveIdsRef.current.length ? lastActiveIdsRef.current : activeKssks;
  ...
  // Single-source finalization on RESULT/DONE OK
  useEffect(() => {
    if (suppressLive) return;
  ...
  }, [serial.lastEvTick, macAddress, suppressLive]); // eslint-disable-line react-hooks/exhaustive-deps

  // Derived-success fallback
  ...
  }, [branchesData, groupedBranches, checkFailures, isScanning, isChecking, macAddress]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const CHECKPOINT_URL =
    process.env.NEXT_PUBLIC_KROSY_URL_CHECKPOINT_ONLINE || "/api/krosy/checkpoint";
  ...
  const sendCheckpointForMac = useCallback(
    async (mac: string, onlyIds?: string[]) => {
      ...
      try {
        ...
        const rList = await fetch(`/api/aliases?mac=${encodeURIComponent(MAC)}&all=1`, {
          cache: "no-store",
        });
        if (!rList.ok) return;
        const j = await rList.json();
        const items: any[] = Array.isArray(j?.items) ? j.items : [];
        let ids = items.map((it) => String((it.ksk ?? it.kssk) || "").trim()).filter(Boolean);

        if (onlyIds && onlyIds.length) {
          const want = new Set(onlyIds.map((s) => s.toUpperCase()));
          ids = ids.filter((id) => want.has(id.toUpperCase()));
          if (ids.length === 0 && items.length) {
            const firstId = String(((items[0] as any)?.ksk ?? (items[0] as any)?.kssk) ?? "")
              .trim();
  ...
  async function clearKskLocksFully(mac: string): Promise<boolean> {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const left = Array.isArray(v?.locks)
        ? v.locks.filter((x: any) => String(x?.mac || "").toUpperCase() === MAC).length
        : 0;
      try {
  ...
  const finalizeOkForMac = useCallback(
    async (rawMac: string) => {
      ...
      try {
        // Show OK SVG overlay, then reset to idle
        setOverlay({ open: true, kind: "success", code: "" });
        setSuppressLive(true);
        try {
          if (DEBUG_LIVE) console.log("[LIVE] OFF → OK latched; suppressing live updates");
        } catch {}
        setMacAddress("");
        setKfbNumber("");
        try {
        ...
        // Hold the OK overlay briefly before full UI reset
    ...
    [handleResetKfb, activeKssks, sendCheckpointForMac]
  );
  ...
  const runCheck = useCallback(
    async (mac: string, attempt: number = 0, pins?: number[]) => {
      ...
      try {
        console.log("[FLOW][CHECK] start", { mac, attempt, pinsCount: pins?.length || 0 });
        try {
          ...
          startTransition(() =>
            setBranchesData((_prev) => {
              ...
              const contactless = new Set<number>(
                (Array.isArray(result?.latchPins) ? (result.latchPins as number[]) : latchPins || []).filter(
              ...
              const filt = (arr: any[]) =>
                activeSet.size
                  ? arr.filter((it) =>
                      activeSet.has(String(((it as any).ksk ?? (it as any).kssk) || "").trim())
                    )
                  : arr;
              ...
              if (items.length) {
                ...
                for (const it of items) {
                  ...
                  const contactless = new Set<number>(
                    (Array.isArray((it as any)?.latchPins) ? (it as any).latchPins : latchPins || []).filter(
                      (n: number) => Number.isFinite(n)
                    ) as number[]
                  );
                  const branchesG = pinsG.map((pin) => {
                    const nameRaw = a[String(pin)] || aliases[String(pin)] || "";
                    const name = String(nameRaw || "").startsWith("CL_") ? String(nameRaw) : `PIN ${pin}`;
                    return {
                      id: `${it.ksk}:${pin}`,
                      branchName: name,
                      testStatus: failures.includes(pin)
                ...
                const groups: Array<{ ksk: string; branches: BranchDisplayData[] }> = Array.from(
                  byId.entries()
                )
                ...
                for (const g of groups)
                  for (const b of g.branches) if (typeof b.pinNumber === "number") knownPinsSet.add(b.pinNumber);
                ...
                for (const g of groups)
                  for (const b of g.branches)
                    if (typeof b.pinNumber === "number") unionMap[b.pinNumber] = b.branchName;
    ...
    [finalizeOkForMac, showHeadlineResult]
  );
  ...
  const loadBranchesData = useCallback(
    async (value?: string, source: "scan" | "manual" = "scan") => {
      ...
      if (!isMac && !KFB_REGEX.test(normalized)) {
        setErrorMsg(source === "manual" ? "Invalid code. Expected MAC like AA:BB:CC:DD:EE:FF" : `Invalid code: ${normalized}`);
        console.warn("[FLOW][SCAN] rejected by patterns", { normalized });
        return;
      }
      lastScanRef.current = normalized;
      if (source === "scan") setShowScanUi(true);
      setIsScanning(true);
      try {
        console.log("[FLOW] State → scanning");
      } catch {}
      ...
      try {
        ...
        if (activeIds.length) setActiveKssks(activeIds);
        {
          try {
            ...
            const rAll = await fetch(`/api/aliases?mac=${encodeURIComponent(mac)}&all=1`, {
              cache: "no-store",
            });
            if (rAll.ok) {
              ...
              if (itemsFiltered.length) {
                try {
                  console.log("[FLOW][LOAD] aliases snapshot items", { count: itemsFiltered.length });
                } catch {}
              ...
              for (const it of itemsFiltered) {
                if (Array.isArray(it.normalPins))
                  for (const n of it.normalPins) if (Number.isFinite(n) && n > 0) pinSet.add(Number(n));
                if (Array.isArray(it.latchPins))
                  for (const n of it.latchPins) if (Number.isFinite(n) && n > 0) pinSet.add(Number(n));
              }
              if (pinSet.size && pins.length === 0) pins = Array.from(pinSet).sort((x, y) => x - y);
              if (activeIds.length) {
                try {
                  console.log("[FLOW][LOAD] GET union /api/aliases", { mac });
                  const rUnion = await fetch(`/api/aliases?mac=${encodeURIComponent(mac)}`, {
                    cache: "no-store",
                  });
                  if (rUnion.ok) {
                    ...
                    try {
                      ...
                      pins = Array.from(new Set([...filtered.normal, ...filtered.latch])).sort(
                        (a, b) => a - b
                      );
    ...
    [runCheck, computeActivePins, clearSoftIdleTimer, showHeadlineResult]
  );
  ...
  useEffect(() => {
    ...
    if (want && seen && !pathsEqual(seen, want)) return;
    const code = serial.lastScan;
    if (!code) return;
    ...
    const tick = async () => {
      try {
        ...
        if (res.ok) {
          ...
          try {
            if (typeof retryInMs === "number") (window as any).__scannerRetry = retryInMs;
          } catch {}
  ...
  const appCurrentViewType =
    mainView === "settingsConfiguration" || mainView === "settingsBranches" ? "settings" : "main";
  ...
  return (
    <div className="relative flex min-h-screen bg-white">
              ...
              {(() => {
                ...
                return (
                  <BranchDashboardMainContent
                    ...
                    kfbNumber={kfbNumber}
                    kfbInfo={kfbInfo}
                    isScanning={isScanning && showScanUi} // Headline: "SCANNING…" when true
                    macAddress={macAddress} // MAC title when scanning or active
                    ...
                    scanResult={scanResult} // Headline: "NOTHING…" or "ERROR" for 2s
                  />
                );
              })()}
      ...
      {/* Success-only OK overlay with SVG. No scanning/error overlays. */}
      <AnimatePresence>
