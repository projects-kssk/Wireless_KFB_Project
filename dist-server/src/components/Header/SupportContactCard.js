...
export const SupportContactCard = ({ supportInfo, size = "lg", variant = "pill", emphasis = false }) => {
    ...
    return (<m.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} className={variant === "pill" ? pillClasses : cardClasses} style={emphasis
      ...
      <div className={`inline-flex items-center ${sizeClasses.gap}`}>
        ...
        <span className={[
            ...
            sizeClasses.label,
        ].join(" ")} style={{ letterSpacing: "0.08em" }} aria-live="polite">
          <span>{label.toUpperCase()}:</span>
          {typeof count === "number" && (<span className={`ml-2 tabular-nums ${sizeClasses.countInside}`}>{count}</span>)}
        </span>

        {/* Subtitle (optional) */}
        {subtitle && (<span className={`opacity-80 ${sizeClasses.subtitle}`}>{subtitle}</span>)}

        {/* CTA (optional) */}
        {ctaText && (<button onClick={onCta} className={[
