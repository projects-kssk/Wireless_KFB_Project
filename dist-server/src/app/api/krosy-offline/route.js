...
function padKey(k, w = 14) {
    return (k + ":").padEnd(w + 1, " ");
}
function buildReport(a) {
    const statusLabel = a.httpStatus >= 200 && a.httpStatus < 300 ? "OK" : a.httpStatus ? "ERROR" : "NO RESPONSE";
...
function hasWorkingDataTag(xml) {
    return WORKINGDATA_RE.test(xml || "");
}
function isCompleteKrosy(xml) {
...
export async function POST(req) {
    ...
    try {
        log.info('offline.forward.config', { OGLIEN_URL, targetUrl });
    }
    catch { }
    ...
    try {
        upstreamJson = text && text.trim().startsWith("{") ? JSON.parse(text) : null;
    }
    catch { }
    const responsePretty = upstreamJson ? "" : prettyXml(text || "");
    const hasWorkingData = upstreamJson
    ...
    try {
        await (async function pruneOldLogs(root, maxAgeDays = 31) {
            try {
                const now = Date.now();
