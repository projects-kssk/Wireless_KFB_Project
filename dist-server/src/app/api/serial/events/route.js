...
import { listSerialDevices, ensureScanners, considerDevicesForScanner, espHealth, } from '@/lib/serial';
...
async function ethStatus() {
    ...
    const ip = (ifaces[iface] || [])
        .filter((x) => x.family === 'IPv4' && !x.internal)
        .map((x) => x.address)[0] ?? null;
...
export async function GET(req) {
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            const send = (obj) => push(`data: ${JSON.stringify(obj)}\n\n`);
            const comment = (txt) => push(`: ${txt}\n\n`);
            try {
                req.signal?.addEventListener('abort', () => { cleanup(); try {
                    controller.close();
                }
                catch { } });
            }
            ...
            // initial payloads
            try {
                send({ type: 'net', ...(await ethStatus()) });
            }
            catch { }
            try {
                const r = getRedis();
                const d = redisDetail();
                send({ type: 'redis', ready: !!(r && (r.status === 'ready')), status: r?.status, detail: d });
            }
            catch { }
            ...
            try {
                ...
                ensureScanners(allPaths).catch((err) => {
                ...
                const onLine = (buf) => {
                    try {
                        ...
                        if ((m = line.match(/\bEV\s+([PL])\s+(\d{1,3})\s+([01])\s+([0-9A-F:]{17})/i))) {
                            ...
                            try {
                                console.log('[events] EV', { kind, ch, val, mac, line });
                            }
                            catch { }
                            ...
                            if (macAllowed(mac)) {
                                try {
                                    ...
                                    if (__LAST_LOG.shouldLog(key))
                                        console.log('[events] EV DONE', { ok, mac, line });
                                }
                                catch { }
