...
// --- SVG ICONS ---
const CheckCircleIcon = (props) => (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}>
    <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd"/>
  </svg>);
const XCircleIcon = ({ className = "w-5 h-5" }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
    <circle cx="12" cy="12" r="10"></circle>
    <line x1="15" y1="9" x2="9" y2="15"></line>
    <line x1="9" y1="9" x2="15" y2="15"></line>
  </svg>);
const ClockIcon = ({ className = "w-5 h-5" }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
    <circle cx="12" cy="12" r="10"></circle>
    <polyline points="12 6 12 12 16 14"></polyline>
  </svg>);
const HelpCircleIcon = ({ className = "w-5 h-5" }) => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
    <circle cx="12" cy="12" r="10"></circle>
...
const BranchCardBase = ({ branch }) => {
    ...
    return (<div className="group relative w-full rounded-2xl bg-white backdrop-blur-sm shadow-lg hover:shadow-xl border-2 border-transparent transition-all duration-300 flex flex-col overflow-hidden">
      ...
      <div className="p-3 flex-grow flex flex-col justify-between">
        <div className="flex justify-between items-center mb-3">
          ...
          {branch.pinNumber != null && (<div className="flex items-center gap-2 text-right">
...
const BranchDashboardMainContent = ({ appHeaderHeight, onScanAgainRequest, onManualSubmit, branchesData, isScanning, kfbNumber, kfbInfo, allowManualInput = true, 
// showRemoveCable intentionally ignored
onResetKfb, onFinalizeOk, macAddress, groupedBranches = [], checkFailures = null, nameHints, activeKssks = [], scanningError = false, disableOkAnimation = false, lastEv, lastEvTick, normalPins, latchPins, forceOkTick, flashOkTick, okSystemNote, }) => {
    // Lifecycle logs for live-session enter/exit based on MAC binding
    const prevMacRef = useRef("");
    useEffect(() => {
    ...
    const [localBranches, setLocalBranches] = useState(branchesData);
    const [recentMacs, setRecentMacs] = useState([]);
    const lastForcedOkRef = useRef(0);
    ...
    const busyEnterTimer = useRef(null);
    const clearBusyTimer = useRef(null);
    ...
    const pinStateRef = useRef(new Map());
    ...
    useEffect(() => {
        ...
        if (snap === lastPropsSnapRef.current)
            return;
    ...
    useEffect(() => {
        try {
            const counts = localBranches.reduce((acc, b) => {
    ...
    useEffect(() => {
        ...
        const isLegacyResult = kindRaw === "RESULT" ||
        ...
        const parseFailures = (s) => {
            ...
            if (captured)
                captured.split(/[\s,]+/).forEach(addNum);
            else
                (cleanMacs.match(/\b\d{1,4}\b/g) || []).forEach(addNum);
        ...
        if (kind === "DONE") {
            ...
            const matchMac = !macToCheck || macToCheck === ZERO || macToCheck === current;
            if (!matchMac)
                return;
            const okFlag = String(lastEv.ok).toLowerCase() === "true" || okFromText;
            if (okFlag) {
                const latchSet = new Set(normalizedLatchPins);
        ...
        const expected = new Set(expectedPins);
        ...
        if ((kind === "P" || kind === "L") &&
            ...
            startTransition(() => setLocalBranches((prev) => {
                ...
                const next = prev.map((b) => {
                    ...
                    const nextStatus = val === 1
                        ? "ok"
                        : isLatch
                            ? b.testStatus // ignore downgrades for latch
                            : "nok";
                    if (b.testStatus === nextStatus)
                        return b;
                    changed = true;
                    return { ...b, testStatus: nextStatus };
                });
    ...
    const pending = useMemo(() => localBranches
        .filter((b) => b.testStatus === "nok")
        .sort((a, b) => {
        const ap = typeof a.pinNumber === "number"
            ? a.pinNumber
            : Number.POSITIVE_INFINITY;
        const bp = typeof b.pinNumber === "number"
            ? b.pinNumber
            : Number.POSITIVE_INFINITY;
        if (ap !== bp)
            return ap - bp;
    ...
    const failurePins = useMemo(() => {
        if (Array.isArray(checkFailures) && checkFailures.length > 0) {
            return [
                ...new Set(checkFailures.filter((n) => Number.isFinite(n))),
            ].sort((a, b) => a - b);
        }
        const pins = pending
            .map((b) => b.pinNumber)
    ...
    const isLatchPin = useCallback((p) => typeof p === "number" && normalizedLatchPins.includes(p), [normalizedLatchPins]);
    // All-OK gates
    const flatAllOk = useMemo(() => settled &&
    ...
    const groupedAllOk = useMemo(() => {
        ...
        for (const b of localBranches)
            if (typeof b.pinNumber === "number")
                byPin.set(b.pinNumber, b.testStatus);
        return groupedBranches.every((g) => g.branches.length > 0 &&
            g.branches.every((b) => {
                const p = b.pinNumber;
                ...
                const isLatch = b.isLatch === true || isLatchPin(p);
    ...
    const clearedMacsRef = useRef(new Set());
    useEffect(() => {
        const mac = (macAddress || "").toUpperCase();
        if (!settled || !allOk || !mac)
            return;
        if (clearedMacsRef.current.has(mac))
            return;
    ...
    useEffect(() => {
        ...
        if (!t || t === lastForcedOkRef.current)
            return;
    ...
    const okBoardRef = useRef("");
    const lastFlashTickRef = useRef(0);
    const queuedFlashTickRef = useRef(0);
    const triggerOkFlash = useCallback((tick) => {
        if (tick === lastFlashTickRef.current)
            return; // de-dupe
    ...
    const runCheck = useCallback(async () => {
        ...
        try {
            ...
            startTransition(() => setLocalBranches((prev) => prev.map((b) => {
                if (typeof b.pinNumber !== "number" || b.notTested)
                    return b;
                const pin = b.pinNumber;
                ...
                if (contactless.has(pin))
                    return { ...b, testStatus: "not_tested" };
                return { ...b, testStatus: "ok" };
            })));
        }
        catch (e) {
    ...
    const onMacChange = (v) => setInputValue(formatMac(v));
    const macValid = MAC_RE.test(inputValue.trim());
    // --- STATUS PILL ---
    const StatusPill = () => {
        if (isChecking) {
            return (<m.span className="inline-flex items-center gap-2 rounded-full border border-amber-300 bg-amber-50 text-amber-700 px-3 py-1 text-xs font-bold" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }}>
          <m.span className="w-2 h-2 rounded-full bg-amber-500" animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1.2, ease: "easeInOut" }}/>
          CHECKING
        </m.span>);
        }
        if (isScanning) {
            return (<m.span className="inline-flex items-center gap-2 rounded-full border border-blue-300 bg-blue-50 text-blue-700 px-3 py-1 text-xs font-bold" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }}>
          <m.span className="w-2 h-2 rounded-full bg-blue-500" animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1.2, ease: "easeInOut" }}/>
          SCANNING…
        </m.span>);
        }
        return null;
    ...
    const mainContent = () => {
        if (scanningError) {
            return (<div className="p-10 text-center w-full flex flex-col items-center justify-center">
        ...
        if (busy) {
            ...
            return (<div className="flex flex-col items-center justify-center h-full min-h-[500px]" aria-busy="true" aria-live="polite">
        ...
        if (showOkAnimation) {
            ...
            return (<div className="p-10 text-center w-full flex flex-col items-center justify-center select-none">
          <div className="relative">
            <m.div className="relative w-80 h-80 sm:w-[360px] sm:h-[360px] rounded-full flex items-center justify-center" initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 240, damping: 22 }}>
          ...
          {okSystemNote && (<div className="mt-1 text-slate-400 text-base">{okSystemNote}</div>)}
        </div>);
        }
        if (hasMounted && localBranches.length === 0) {
            if (isManualEntry) {
                return (<div className="flex flex-col items-center justify-center h-full min-h-[500px] w-full max-w-3xl p-0">
            <div className="relative w-full rounded-3xl border border-slate-200/80 shadow-2xl overflow-hidden bg-white/90">
              <button type="button" onClick={() => setIsManualEntry(false)} aria-label="Close" className="absolute top-3 right-3 z-10 inline-flex items-center justify-center h-12 w-12 rounded-full border-2 border-slate-300 bg-white text-slate-800 hover:bg-slate-100 shadow" title="Close">
                ...
                <form onSubmit={handleManualSubmit} className="w-full grid gap-6">
                  <div className="grid gap-2">
                    ...
                    <div className={[
                      ...
                      <input type="text" value={inputValue} onChange={(e) => onMacChange(e.target.value)} placeholder="08:3A:8D:15:27:54" inputMode="text" autoCapitalize="characters" spellCheck={false} maxLength={17} pattern="^([0-9A-F]{2}:){5}[0-9A-F]{2}$" className={[
                    ...
                    ].join(" ")} autoFocus aria-invalid={!macValid && !!inputValue} aria-describedby="mac-help"/>
                    ...
                    {!macValid && inputValue && (<div className="text-center text-red-600 font-bold">
                        Invalid MAC format
                      </div>)}
                  </div>
                  {recentMacs.length > 0 && (<div className="flex flex-wrap items-center justify-center gap-2">
                      <span className="text-slate-500 font-semibold mr-2">
                        Recent:
                      </span>
                      {recentMacs.map((m) => (<button key={m} type="button" onClick={() => onMacChange(m)} className="px-3 py-1 rounded-full border border-slate-200 bg-white hover:bg-slate-100 font-mono text-slate-700" title={m}>
                          {m}
                        </button>))}
                    </div>)}
                  <div className="grid gap-4">
                    <button type="submit" disabled={!macValid || busy} className={[
            ...
            // Scan box
            return (<div className="flex flex-col items-center justify-center h-full min-h-[520px]">
          <div className="w-full flex flex-col items-center gap-8">
            ...
            {isScanning && (<div className="flex flex-col items-center gap-3">
                <p className="text-slate-600 text-3xl md:text-4xl font-bold tracking-wide">
                  SCANNING…
                </p>
                <m.div className="h-1 w-56 md:w-72 rounded-full bg-slate-300/50 overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}>
                  <m.div className="h-full w-1/3 bg-blue-500" animate={{ x: ["-20%", "100%"] }} transition={{
                        repeat: Infinity,
                        duration: 1.2,
                        ease: "easeInOut",
                    }}/>
          ...
          {allowManualInput && !isScanning && (<button onClick={() => setIsManualEntry(true)} className="mt-10 text-xl md:text-2xl text-slate-500 hover:text-blue-600 transition-colors underline">
        ...
        if (groupedBranches && groupedBranches.length > 0) {
            ...
            const statusByPin = new Map();
            for (const b of localBranches)
                if (typeof b.pinNumber === "number")
                    statusByPin.set(b.pinNumber, b.testStatus);
            const ksskCards = groupedBranches.map((grp) => {
                ...
                const okBranches = branchesLive.filter((b) => {
                    if (b.testStatus !== "ok" || typeof b.pinNumber !== "number")
                        return false;
                    const isContactless = b.isLatch === true || isLatchPin(b.pinNumber);
                    const noCheck = b.noCheck === true || b.notTested === true;
                    return !(isContactless || noCheck);
                });
                const okNames = okBranches
                    .map((b) => nameHints && b.pinNumber != null && nameHints[String(b.pinNumber)]
                ...
                const failedItems = branchesLive
                    .filter((b) => typeof b.pinNumber === "number" &&
                    (b.testStatus === "nok" ||
                        (b.testStatus !== "ok" &&
                            (b.isLatch === true || isLatchPin(b.pinNumber)))))
                    .map((b) => ({
                    pin: b.pinNumber,
                    name: nameHints && b.pinNumber != null && nameHints[String(b.pinNumber)]
                        ? nameHints[String(b.pinNumber)]
                        : b.branchName,
                    isLatch: b.isLatch === true || isLatchPin(b.pinNumber),
                }))
                    .sort((a, b) => a.name.localeCompare(b.name));
                const missingNames = failedItems.map((f) => f.name);
                const activeSet = new Set((activeKssks || []).map(String));
                const isActive = activeSet.has(String(grp.ksk ?? ""));
                return (<section key={grp.ksk} className="rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow">
            <header className="px-4 py-3 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className={`inline-flex items-center justify-center w-10 h-10 rounded-full ${isActive ? "bg-blue-600" : "bg-blue-500"} text-white font-extrabold shadow`}>
                  {String(grp.ksk).slice(-2)}
                </span>
            ...
            <div className="p-4 grid gap-4">
              {failedItems.length > 0 && (<div>
                  <div className="text-[12px] font-bold uppercase text-slate-600 mb-2">
                    Missing items
                  </div>
                  <div className="grid gap-2">
                    {failedItems.map((f) => (<div key={`f-${grp.ksk}-${f.pin}`} className="rounded-xl border border-red-200 bg-red-50/40 p-3">
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          ...
                          {f.isLatch && (<span className="inline-flex items-center rounded-full bg-amber-50 text-amber-800 border border-amber-200 px-2 py-[3px] text-[11px]" title="Contactless pin">
                              Contactless
                            </span>)}
                        </div>
                      </div>))}
              ...
              {okNames.length > 0 && (<div>
                  <div className="text-[12px] font-bold uppercase text-slate-600 mb-2">
                    Passed
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {okNames.slice(0, 24).map((nm, i) => (<span key={`ok-${grp.ksk}-${i}`} className="inline-flex items-center rounded-full bg-slate-50 text-slate-500 border border-slate-200 px-2 py-[5px] text-[12px] font-semibold">
                        {nm}
                      </span>))}
                    {okNames.length > 24 && (<span className="text-[11px] text-slate-500">
            ...
            return (<div className="flex flex-col gap-4 w-full mt-0">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
          {pending.map((branch) => (<BranchCard key={branch.id} branch={branch}/>))}
        </div>
      </div>);
    };
    ...
    useEffect(() => {
        try {
            if (viewKey === "scan")
                console.log("[LIVE] OFF → scan view");
            else
                console.log("[LIVE][VIEW]", { viewKey });
        }
        catch { }
    }, [viewKey]);
    return (<div className="flex-grow flex flex-col items-center justify-start p-2">
      <header className="w-full mb-1 min-h-[56px]">
        {kfbInfo?.board ||
            kfbNumber ||
            (macAddress && localBranches.length > 0) ? (<div className="flex items-center justify-between gap-1">
            {macAddress || kfbInfo?.board || kfbNumber ? (<div className="flex items-center gap-3">
            ...
            {macAddress && localBranches.length > 0 && (<div className="flex items-center justify-end gap-4 w-full">
                {!showingGrouped && (<div className="flex flex-col items-end leading-tight mt-2 pt-2 border-t border-slate-200/70">
                    <div className="text-sm md:text-base uppercase tracking-wide text-slate-600">
                      Active KSKs
                    </div>
                    <div className="flex flex-wrap gap-2 mt-1 justify-end">
                      {activeKssks && activeKssks.length > 0 ? (activeKssks.map((id) => (<span key={id} className="inline-flex items-center rounded-lg border border-slate-400 bg-white text-slate-800 px-4 py-2 text-lg md:text-xl font-extrabold shadow">
                            {id}
                          </span>))) : (<span className="text-slate-400 text-xs">—</span>)}
                    </div>
                  </div>)}
              </div>)}
          </div>) : null}
      ...
      <AnimatePresence mode="wait">
        <m.div key={viewKey} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18, ease: "easeOut" }} className="w-full">
          {mainContent()}
        </m.div>
      </AnimatePresence>
