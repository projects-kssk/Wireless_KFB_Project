...
function parseResult(line: string): { ok: boolean; missing: number[]; extra: number[] } | null {
  const upper = line.toUpperCase();
  if (/\bSUCCESS\b/.test(upper)) return { ok: true, missing: [], extra: [] };

  // FAILURES: 1,2
  const m1 = upper.match(/FAILURES:\s*([0-9,\s]+)/);
  if (m1) return { ok: false, missing: parseNums(m1[1]), extra: [] };
...
async function writeLine(line: string): Promise<void> {
  const { port } = getEspLineStream() as any; // { parser, port }
  await new Promise<void>((resolve, reject) => port.write(line, (e: any) => (e ? reject(e) : resolve())));
}

function waitForResult(signal: AbortSignal, mac: string, timeoutMs = 15000):
  Promise<{ raw: string; parsed: { ok: boolean; missing: number[]; extra: number[] } }> {
  return new Promise((resolve, reject) => {
    const { parser } = getEspLineStream() as any;
    let timer: any;

    const done = (val?: any, err?: any) => {
    ...
    const onData = (buf: Buffer | string) => {
      ...
      if (macMatch && macMatch[1] !== mac) return;

      const parsed = parseResult(raw);
      if (parsed) return done({ raw, parsed });
    };
...
export async function POST(req: Request) {
  try {
    const { mac, pins } = (await req.json().catch(() => ({}))) as { mac?: string; pins?: number[] };
    ...
    const body: any = { failures: parsed.ok ? [] : parsed.missing };
    if (parsed.extra.length) body.extra = parsed.extra;
