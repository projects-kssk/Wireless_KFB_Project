...
const normMac = (s) => s.trim().toUpperCase();
const escMac = (s) => esc(normMac(s));
function buildMatchers(macUp) {
    ...
    const REPLY_RESULT_RE = new RegExp(`^\\s*←\\s*(?:reply|resp|response)\\s+from\\s+${MAC}\\s*:\\s*(?:RESULT\\s+)?(SUCCESS|OK|FAIL(?:URE)?[^\\n]*)`, 'i');
    ...
    const isResult = (s) => RESULT_RE.test(s) || REPLY_RESULT_RE.test(s);
    return {
        SENT_RE,
        RESULT_RE,
        REPLY_RESULT_RE,
...
function parseFailuresFromLine(line, pins) {
    const want = Array.isArray(pins) && pins.length > 0 ? new Set(pins) : null;
...
function getTail() {
    try {
        const s = serial.getEspLineStream?.();
...
export async function POST(request) {
    ...
    try {
        ...
        let clientPins = Array.isArray(parsed.data.pins)
        ...
        try {
            ...
            const members = await r.smembers(indexKey).catch(() => []);
            const stationId = (process.env.STATION_ID || process.env.NEXT_PUBLIC_STATION_ID || '').trim();
            const act = stationId ? await r.smembers(`ksk:station:${stationId}`).catch(() => []) : [];
            // Union of station-active and indexed KSKs
            let targets = Array.from(new Set([...(Array.isArray(act) ? act : []), ...(Array.isArray(members) ? members : [])])).filter(Boolean);
            ...
            if (!targets.length) {
                try {
                    ...
                    if (typeof r.scan === 'function') {
                        ...
                        const keys = await r.keys(pattern).catch(() => []);
                        for (const k of keys) {
                            const id = String(k).slice(pattern.length - 1).replace(/^:/, '');
                            if (id)
                                found.push(id);
            ...
            const unionNames = {};
            const unionN = new Set();
            const unionL = new Set();
            const hadActiveTargets = Array.isArray(act) && act.length > 0;
            for (const id of targets) {
                try {
                    ...
                    for (const [k, v] of Object.entries(names)) {
                        if (!unionNames[k])
                            unionNames[k] = String(v);
                    }
                    if (Array.isArray(d?.normalPins))
                        for (const n of d.normalPins) {
                            const x = Number(n);
                    ...
                    if (Array.isArray(d?.latchPins))
                        for (const n of d.latchPins) {
                            const x = Number(n);
                            if (Number.isFinite(x) && x > 0) {
                                pinsSet.add(x);
                                unionL.add(x);
            ...
            try {
                ...
                try {
                    broadcast({ type: 'aliases/union', mac: macUp, names: outNames, normalPins: nArr, latchPins: lArr });
                }
                catch { }
            }
            catch { }
        ...
        let pins = undefined;
        const uniqSort = (arr) => Array.from(new Set((arr || []).filter((n) => Number.isFinite(n) && n > 0))).sort((a, b) => a - b);
        if (SEND_MODE === 'merge') {
            const merged = uniqSort([...(pinsUnion || []), ...(clientPins || [])]);
            if (merged.length)
                pins = merged;
        }
        else if (SEND_MODE === 'client') {
            if (Array.isArray(clientPins) && clientPins.length)
                pins = uniqSort(clientPins);
            else if (pinsUnion.length)
                pins = uniqSort(pinsUnion);
        }
        ...
        if (!pins) {
            ...
            if (rawDef) {
                ...
                if (defPins.length)
                    pins = Array.from(new Set(defPins)).sort((a, b) => a - b);
            }
            // if still no pins → MAC-only
        }
        const { sendToEsp, waitForNextLine } = serial;
        ...
        if ((process.env.LOG_MONITOR_START_ONLY ?? '0') !== '1')
            mon.info(`CHECK start mac=${macUp} mode=${SEND_MODE} pins=${pins?.length ?? 0}`);
        const { SENT_RE, RESULT_RE, REPLY_RESULT_RE, IGNORED_RE, INVALID_RE, ADDPEER_RE, SENDFAIL_RE, isResult, OK_ONLY_RE, } = buildMatchers(macUp);
        ...
        try {
            ...
            if (espPath && (process.env.LOG_MONITOR_START_ONLY ?? '0') !== '1')
                mon.info(`CHECK espPath=${espPath}`);
        }
        catch { }
        ...
        const pIgnored = waitForNextLine(IGNORED_RE, signal, RESULT_TIMEOUT_MS).then((s) => {
            const m = s.match(IGNORED_RE);
            throw new Error(`mac-mismatch expected ${macUp} got ${m?.[1] ?? '?'}`);
        });
        ...
        let line = null;
        try {
            line = String(await Promise.race([pResult, pIgnored, pInvalid, pAddPeer, pSendFail, pOkOnly])).trim();
            ...
            if (String(e?.message ?? e) === 'timeout') {
                ...
                const REPLY_RESULT_RE = new RegExp(`^\\s*←\\s*reply\\s+from\\s+${MAC}\\s*:\\s*(?:RESULT\\s+)?(SUCCESS|FAILURE[^\\n]*)`, 'i');
        ...
        if (!line) {
            ...
            return NextResponse.json({ error: 'no-result-yet', code: 'NO_RESULT', symbol: 'X', pending: true, ...(aliasesFromRedis ? { aliases: aliasesFromRedis } : {}), ...(pinMeta || {}), ...(itemsAll ? { items: itemsAll } : {}) }, { status: 504, headers: { 'X-Req-Id': rid } });
        }
        // Terminal parse
        ...
        if (m1 && /^SUCCESS|OK/i.test(m1[1])) {
            ...
            try {
                void sendCheckpointFromAliases(macUp, rid);
            }
            catch { }
            ...
            try {
                ...
                if (r) {
                    ...
                    const stationSetKeys = [];
                    // 1) Include explicit current station if set
                    if (explicitStation)
                        stationSetKeys.push(`ksk:station:${explicitStation}`);
                        ...
                        if (typeof r.scan === 'function') {
                            ...
                            do {
                                ...
                                for (const k of chunk)
                                    if (!stationSetKeys.includes(k))
                                        stationSetKeys.push(k);
                            } while (cursor !== '0');
                        }
                        else {
                            const keys = await r.keys('ksk:station:*').catch(() => []);
                            for (const k of keys)
                                if (!stationSetKeys.includes(k))
                                    stationSetKeys.push(k);
                        }
                    ...
                    for (const setKey of stationSetKeys) {
                        try {
                            const members = await r.smembers(setKey).catch(() => []);
                    ...
                    try {
                        ...
                        if (typeof r.scan === 'function') {
                            ...
                            const keys = await r.keys('ksk:*').catch(() => []);
                            lockKeys.push(...keys);
                        }
                        for (const key of lockKeys) {
                            try {
                                ...
                                if (macLock === macUp) {
                                    ...
                                    if (sid)
                                        await r.srem(`ksk:station:${sid}`, String(v.kssk || '').trim()).catch(() => { });
                                }
                            }
                            catch { }
                        }
        ...
        try {
            const { getRedis } = await import('@/lib/redis');
            const r = getRedis();
            const rawUnion = await r.get(`kfb:aliases:${macUp}`).catch(() => null);
            if (rawUnion) {
        ...
        try {
            ...
            if (raw) {
                ...
                if (parsedR?.names && typeof parsedR.names === 'object')
                    aliasesFromRedis = parsedR.names;
                ...
                if (parsedR?.hints && typeof parsedR.hints === 'object') {
                    ...
                    pinMeta.nameHints = parsedR.hints;
                }
            }
            const members = await r.smembers(`kfb:aliases:index:${macUp}`).catch(() => []);
            ...
            try {
                ...
                if (stationId && itemsAll && itemsAll.length) {
                    const activeIds = await r.smembers(`ksk:station:${stationId}`).catch(() => []);
                    if (activeIds && activeIds.length) {
                        const set = new Set(activeIds.map(String));
                        const filt = itemsAll.filter(it => set.has(String(it.kssk)));
                        if (filt.length)
                            pinMeta = { ...(pinMeta || {}), itemsActive: filt };
                    }
                }
            }
            catch { }
        ...
        try {
            if (msg === 'client-abort' || msg === 'timeout') {
                ...
                for (let i = tail.length - 1; i >= 0; i--) {
                    ...
                    if (m && (/^FAILURE/i.test(m[1]) || /^SUCCESS/i.test(m[1]))) {
                        line = ln;
                        break;
                    }
                }
                if (line) {
                    // Parse failures
                    const failures = parseFailuresFromLine(line, Array.isArray(parsed?.data?.pins) ? parsed.data.pins : undefined);
...
async function sendCheckpointFromAliases(macUp, requestId) {
    try {
        const { getRedis } = await import('@/lib/redis');
        const r = getRedis();
        const raw = await r.get(`kfb:aliases:${macUp}`).catch(() => null);
        if (!raw)
            return;
        const data = JSON.parse(raw);
        const hints = (data?.hints && typeof data.hints === 'object') ? data.hints : {};
        const namesMap = (data?.names && typeof data.names === 'object') ? data.names : {};
        // Prefer CL name hints; fall back to alias names
        let names = [];
        const seen = new Set();
        const pushName = (nm) => { if (!nm)
            return; const key = String(nm).trim(); if (!key)
            return; if (seen.has(key))
            return; seen.add(key); names.push(key); };
        ...
        const seqXml = names.map((nm, i) => (`<sequence index="${i + 1}" compType="clip" reference="1" result="true">` +
            `<objGroup>CL</objGroup><objPos>${escapeXml(nm)}</objPos>` +
            `</sequence>`)).join('');
        const workingDataXml = `<krosy xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://www.kroschu.com/kroscada/namespaces/krosy/visualcontrol/V_0_1" xmlns:xsd="http://www.w3.org/2001/XMLSchema">` +
