...
function writeWithTimeout(port: any, data: string, ms = 1500) {
  return new Promise<void>((resolve, reject) => {
    let done = false;
    const to = setTimeout(() => { if (!done) { done = true; reject(new Error("serial-write-timeout")); } }, ms);
...
function drainSoft(port: any, ms = 300) {
  if (typeof port?.drain !== "function") return Promise.resolve();
  return new Promise<void>((resolve) => {
    let settled = false;
    const to = setTimeout(() => { if (!settled) { settled = true; resolve(); } }, ms);
    port.drain(() => {
      if (settled) return;
      clearTimeout(to);
      settled = true;
...
function waitForAck(signal: AbortSignal, wantMacUpper: string, softTimeoutMs = 10000) {
  ...
  return new Promise<{ hubMac: string; status: StatusT; raw: string }>((resolve, reject) => {
    ...
    for (let i = ring.length - 1; i >= 0; i--) {
      const p = parse(ring[i]); if (!p) continue;
      if (p.hasAckWelcome && p.status) return resolve({ hubMac: p.mac || wantMacUpper, status: p.status, raw: p.raw });
      if (p.hasWelcome) {
        for (let j = i + 1; j < ring.length; j++) {
          const q = parse(ring[j]); if (q?.status) return resolve({ hubMac: q.mac || wantMacUpper, status: q.status, raw: `${p.raw}\n${q.raw}` });
        }
      }
    }

    let armed = false, lastWelcome: string | null = null;
    const onData = (buf: Buffer | string) => {
      const p = parse(buf); if (!p) return;
      if (p.hasAckWelcome && p.status) { cleanup(); return resolve({ hubMac: p.mac || wantMacUpper, status: p.status, raw: p.raw }); }
      if (p.hasWelcome) { armed = true; lastWelcome = p.raw; return; }
      if (p.status) { cleanup(); return resolve({ hubMac: p.mac || wantMacUpper, status: p.status, raw: lastWelcome ? `${lastWelcome}\n${p.raw}` : p.raw }); }
    };

    const onAbort = () => { cleanup(); reject(new Error("client-abort")); };
    const onTimeout = () => { cleanup(); reject(new Error("timeout")); };
    const cleanup = () => { try { parser?.off?.("data", onData); } catch {} try { signal.removeEventListener("abort", onAbort); } catch {} if (t) clearTimeout(t); };
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const waitP = waitForAck((req as any).signal, mac, 10000);
    await writeWelcome(mac);                // will not hang on drain
    const { hubMac, status, raw } = await waitP;

    if (status === "READY") return NextResponse.json({ ok: true, mac, hubMac, message: `WELCOME ACK from hub ${hubMac} (READY)`, raw });
