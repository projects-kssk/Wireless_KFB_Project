...
async function hasSetupDataForMac(mac) {
    try {
...
function compileRegex(src, fallback) {
    if (!src)
        return fallback;
    try {
...
const MainApplicationUI = () => {
    ...
    const fadeTransition = { duration: reduce ? 0 : 0.18 };
    ...
    const [kfbInfo, setKfbInfo] = useState(null);
    const [macAddress, setMacAddress] = useState("");
    const [isScanning, setIsScanning] = useState(false);
    const [showScanUi, setShowScanUi] = useState(false);
    const [errorMsg, setErrorMsg] = useState(null);
    const [nameHints, setNameHints] = useState(undefined);
    const [normalPins, setNormalPins] = useState(undefined);
    const [latchPins, setLatchPins] = useState(undefined);
    const [activeKssks, setActiveKssks] = useState([]);
    ...
    const finalizeOkGuardRef = useRef(new Set());
    // Check flow
    const [checkFailures, setCheckFailures] = useState(null);
    ...
    const computeActivePins = useCallback((items, activeIds) => {
        const ids = new Set((activeIds || []).map((s) => String(s).trim()));
        const n = new Set();
        const l = new Set();
        if (Array.isArray(items) && ids.size) {
            for (const it of items) {
                const id = String((it?.ksk ?? it?.kssk) || '').trim();
                if (!id || !ids.has(id))
                    continue;
    ...
    // Settings flow
    const [currentConfigIdForProgram, setCurrentConfigIdForProgram] = useState(null);
    ...
    const showOverlay = (kind, code) => setOverlay({ open: true, kind, code });
    ...
    const okResetTimerRef = useRef(null);
    ...
    const forceResetTimer1Ref = useRef(null);
    const forceResetTimer2Ref = useRef(null);
    const forceResetOnce = (primaryMs = 700, fallbackMs = 2200) => {
        if (forceResetDoneRef.current)
            return;
        const fire = () => {
            if (forceResetDoneRef.current)
                return;
    ...
    const [okSystemNote, setOkSystemNote] = useState(null);
    ...
    const scanOverlayTimerRef = useRef(null);
    ...
    const serial = useSerialEvents(suppressLive || !(macAddress && macAddress.trim())
        ? "00:00:00:00:00:00"
        : (macAddress || "").toUpperCase());
    // Log when live stream starts/stops based on mac + suppression
    const liveStateRef = useRef("off");
    const lastLiveMacRef = useRef(null);
    useEffect(() => {
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    if (r.ok) {
                                        ...
                                        if (ids.length) {
                                            try {
                                                console.log('[FLOW][CHECKPOINT] STOP path: sending for all ids', { mac: target, count: ids.length });
                                            }
                                            catch { }
                                            ...
                                            try {
                                                console.log('[FLOW][CHECKPOINT] STOP path: no ids found; skipping');
                                            }
                                            catch { }
                                        }
                                    }
    ...
    const lastScanPath = serial.lastScanPath;
    ...
    const resolveDesiredPath = () => {
        const list = serial.scannerPaths || [];
        if (list[DASH_SCANNER_INDEX])
            return list[DASH_SCANNER_INDEX] || null;
        return null;
    };
    const desiredPath = resolveDesiredPath();
    const desiredTail = (desiredPath || "").split("/").pop() || desiredPath || "";
    const desiredPortState = (() => {
        const map = serial.scannerPorts || {};
        const key = Object.keys(map).find((k) => pathsEqual(k, desiredPath || ""));
        return key
            ? map[key]
            : null;
    })();
    const prevRedisReadyRef = useRef(null);
    const [redisDegraded, setRedisDegraded] = useState(false);
    const redisReadyRef = useRef(false);
    const redisDropTimerRef = useRef(null);
    const lastRedisDropAtRef = useRef(null);
    const macRef = useRef("");
    useEffect(() => {
        redisReadyRef.current = !!serial.redisReady;
    }, [serial.redisReady]);
    useEffect(() => {
        ...
        try {
            ...
            if (prev === true && ready === false) {
                if (redisDropTimerRef.current == null) {
                    ...
                    const detail = serial.redisDetail || {};
                    ...
                    redisDropTimerRef.current = window.setTimeout(() => {
                        ...
                        if (!redisReadyRef.current) {
                            const ms = lastRedisDropAtRef.current
                                ...
                                : undefined;
                            console.warn('[REDIS] degraded mode ON (redisReady=false)', { waitedMs: ms, lastEvent: serial.redisDetail?.lastEvent, lastError: serial.redisDetail?.lastError });
                            setRedisDegraded(true);
                        }
                        else {
                            console.log("[REDIS] recovered before debounce window; staying normal");
                        }
                    }, DEBOUNCE_MS);
                }
            }
            ...
            if (prev === false && ready === true) {
                ...
                console.log('[REDIS] redisReady back to true (degraded OFF)', { downMs: msDown, lastEvent: serial.redisDetail?.lastEvent });
                setRedisDegraded(false);
            }
        }
        catch { }
    }, [serial.redisReady, macAddress]);
    const CLEAR_LOCAL_ALIAS = String(process.env.NEXT_PUBLIC_ALIAS_CLEAR_ON_READY || "").trim() === "1";
    useEffect(() => {
        const u = serial.lastUnion;
        if (!u)
            return;
        if (suppressLive)
            return; // hard gate: ignore union updates after OK
        const cur = (macAddress || "").toUpperCase();
        if (!cur || String(u.mac || "").toUpperCase() !== cur)
            return;
        try {
            const np = Array.isArray(u.normalPins) ? u.normalPins.length : 0;
            const lp = Array.isArray(u.latchPins) ? u.latchPins.length : 0;
            const nm = u.names && typeof u.names === "object"
            ...
            const actIds = (lastActiveIdsRef.current && lastActiveIdsRef.current.length)
                ...
                : activeKssks;
            const fromItems = computeActivePins(itemsAllFromAliasesRef.current, actIds);
            setNormalPins(fromItems.normal);
            setLatchPins(fromItems.latch);
            if (u.names && typeof u.names === "object")
                setNameHints(u.names);
        }
        catch { }
    ...
    useEffect(() => {
        if (redisDegraded)
            return;
        if (suppressLive)
            return; // hard gate during post-OK
            ...
            try {
                ...
                if (r.ok) {
                    ...
                    if (Array.isArray(j?.normalPins))
                        setNormalPins(j.normalPins);
                    if (Array.isArray(j?.latchPins))
                        setLatchPins(j.latchPins);
                    if (j?.aliases && typeof j.aliases === "object")
                        setNameHints(j.aliases);
                }
            }
            catch { }
        })();
    ...
    useEffect(() => {
        if (suppressLive)
            return; // hard gate: ignore SSE after OK is latched
        const ev = serial.lastEv;
        if (!ev)
            return;
        const raw = String(ev.line ?? ev.raw ?? "");
        const kind = String(ev.kind || "").toUpperCase();
        const ok = (/\bRESULT\b/i.test(raw) && /\b(SUCCESS|OK)\b/i.test(raw)) ||
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (ev && cur) {
                ...
                const macMatch = !evMac ||
                    evMac === ZERO ||
                    evMac === cur ||
                    /reply\s+from\s+([0-9A-F]{2}(?::[0-9A-F]{2}){5})/i.test(raw);
                if (macMatch && (isDoneFail || (isResult && isFailText)))
                    return; // abort force OK
            }
        }
        catch { }
        if (okForcedRef.current)
            return;
        const anyFailures = Array.isArray(checkFailures) && checkFailures.length > 0;
        if (anyFailures)
            return;
        const flatOk = Array.isArray(branchesData) &&
            branchesData.length > 0 &&
            branchesData.every((b) => b.testStatus === "ok");
        const groupedOk = Array.isArray(groupedBranches) &&
            groupedBranches.length > 0 &&
            groupedBranches.every((g) => g.branches.length > 0 &&
        ...
        const tick = async () => {
            try {
                ...
                if (!r.ok)
                    return;
                const j = await r.json();
                const rows = Array.isArray(j?.locks) ? j.locks : [];
                const ids = rows
                    .map((l) => String(l.ksk ?? l.kssk))
                    .filter(Boolean);
                if (!stop && ids.length) {
                    setActiveKssks((prev) => Array.from(new Set([...prev, ...ids])));
                }
            }
            catch { }
        };
    ...
    const lastErrorStampRef = useRef(0);
    const scanInFlightRef = useRef(false);
    const okForcedRef = useRef(false);
    const pendingScansRef = useRef([]);
    const enqueueScan = useCallback((raw) => {
        ...
        if (q.length === 0 || q[q.length - 1] !== code)
            q.push(code);
        if (q.length > 5)
            q.splice(0, q.length - 5);
    }, []);
    ...
    const CHECKPOINT_URL = process.env.NEXT_PUBLIC_KROSY_URL_CHECKPOINT_ONLINE ||
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                if (ip && IP_ONLINE && ip === IP_ONLINE)
                    setKrosyLive(true);
                else if (ip && IP_OFFLINE && ip === IP_OFFLINE)
                    setKrosyLive(false);
            }
            catch { }
        })();
    }, []);
    const checkpointSentRef = useRef(new Set());
    const checkpointMacSentRef = useRef(new Set());
    const checkpointMacPendingRef = useRef(new Set());
    const checkpointBlockUntilTsRef = useRef(0);
    // Track last active KSK ids from CHECK
    const lastActiveIdsRef = useRef([]);
    const sendCheckpointForMac = useCallback(async (mac, onlyIds) => {
        ...
        if (checkpointMacSentRef.current.has(MAC))
            return;
        if (checkpointMacPendingRef.current.has(MAC))
            return; // NEW
        ...
        try {
            ...
            if (onlyIds && onlyIds.length) {
                ...
                if (ids.length === 0 && items.length) {
                    ...
                    const firstId = String(((items[0]?.ksk ?? items[0]?.kssk) ?? '')).trim();
                    ids = [firstId].filter(Boolean);
                }
            }
            ...
            let sent = false;
            for (const id of ids) {
                if (checkpointSentRef.current.has(id))
                    continue;
                // try to include workingDataXml if available
                let workingDataXml = null;
                try {
    ...
    async function clearKskLocksFully(mac) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const left = Array.isArray(v?.locks)
                ? v.locks.filter((x) => String(x?.mac || "").toUpperCase() === MAC)
                    .length
                : 0;
            try {
    ...
    const finalizeOkForMac = useCallback(async (rawMac) => {
        ...
        if (finalizeOkGuardRef.current.has(mac))
            return;
        ...
        try {
            ...
            if (hasSetup) {
                ...
                try {
                    console.log('[FLOW][CHECKPOINT] skip (no setup data found for MAC)');
                }
                catch { }
            ...
            const verifyAliasesEmpty = async () => {
                try {
                    const r = await fetch(`/api/aliases?mac=${encodeURIComponent(mac)}&all=1`, {
                        cache: "no-store",
                    });
                    if (!r.ok)
                        return false;
    ...
    const lastFinalizedMacRef = useRef(null);
    useEffect(() => {
    ...
    const runCheck = useCallback(async (mac, attempt = 0, pins) => {
        if (!mac)
            return;
        setIsChecking(true);
        setScanningError(false);
        ...
        try {
            ...
            try {
                if (Array.isArray(result?.pinsUsed))
                    console.log("[FLOW][CHECK] used pins", result.pinsUsed, "mode", result?.sendMode);
            }
            catch { }
            if (res.ok) {
                ...
                try {
                    const activeIds = Array.isArray(result?.itemsActive)
                        ? result.itemsActive
                            .map((it) => String((it.ksk ?? it.kssk) || "").trim())
                            .filter(Boolean)
                        : [];
                ...
                const hints = result?.nameHints && typeof result.nameHints === "object"
                    ? result.nameHints
                    : undefined;
                setNameHints(hints);
                try {
                ...
                startTransition(() => setBranchesData((_prev) => {
                    ...
                    let aliases = {};
                    const itemsPref = Array.isArray(result?.itemsActive)
                        ? result.itemsActive
                        : Array.isArray(result?.items)
                            ? result.items
                            : null;
                    if (itemsPref) {
                        const mergeAliases = (items) => {
                            const merged = {};
                            for (const it of items) {
                        ...
                        aliases = mergeAliases(itemsPref);
                    }
                    if (!aliases || Object.keys(aliases).length === 0) {
                        ...
                        if (result?.items && Array.isArray(result.items)) {
                            merged = mergeAliases(result.items);
                        }
                        else if (result?.aliases &&
                            typeof result.aliases === "object") {
                            merged = result.aliases;
                        }
                        aliases = merged;
                    }
                    ...
                    pins.sort((a, b) => a - b);
                    const contactless = new Set((Array.isArray(result?.latchPins)
                        ? result.latchPins
                        : latchPins || []).filter((n) => Number.isFinite(n)));
                    ...
                    const itemsActiveArr = Array.isArray(result?.itemsActive)
                        ? result.itemsActive
                        : [];
                    let itemsAllArr = Array.isArray(result?.items)
                        ? result.items
                        : [];
                    // Restrict to currently active KSK ids when available
                    const activeSet = new Set((activeKssks || []).map((s) => String(s).trim()));
                    const filt = (arr) => activeSet.size
                        ? arr.filter((it) => activeSet.has(String((it.ksk ?? it.kssk) || '').trim()))
                        : arr;
                    ...
                    if (items.length) {
                        ...
                        for (const it of items) {
                            ...
                            const contactless = new Set((Array.isArray(it?.latchPins)
                                ? it.latchPins
                                : latchPins || []).filter((n) => Number.isFinite(n)));
                        ...
                        const knownPinsSet = new Set();
                        for (const g of groups)
                            for (const b of g.branches)
                                if (typeof b.pinNumber === "number")
                                    knownPinsSet.add(b.pinNumber);
                        const extraPins = failures.filter((p) => Number.isFinite(p) && !knownPinsSet.has(p));
                        if (extraPins.length) {
                        ...
                        const unionPins = Object.keys(unionMap)
                            .map((n) => Number(n))
                            .sort((x, y) => x - y);
                        const contactless = new Set((latchPins || []).filter((n) => Number.isFinite(n)));
                    ...
                    const knownFlat = new Set(pins);
                    const extras = failures.filter((p) => Number.isFinite(p) && !knownFlat.has(p));
                ...
                if (!unknown && failures.length === 0) {
                    ...
                    const rawLine = typeof result?.raw === "string"
                        ? String(result.raw)
                        : null;
                    const msg = rawLine ||
                        (unknown
    ...
    const loadBranchesData = useCallback(async (value, source = "scan") => {
        try {
        ...
        setAwaitingRelease(false);
        try {
            const mac = isMac ? macCanon : normalized; // use normalized MAC when available
            ...
            let activeIds = await (async () => {
                try {
                    ...
                    const list = rows
                        ...
                        .map((l) => String(l.ksk ?? l.kssk).trim())
                        .filter(Boolean);
                ...
                try {
                    ...
                    if (rAll.ok) {
                        ...
                        if (itemsFiltered.length) {
                            ...
                            const groupsRaw = itemsFiltered.map((it) => {
                                const a = it.aliases || {};
                                ...
                                const idStr = String((it.ksk ?? it.kssk) || "");
                        ...
                        if (activeIds.length) {
                            try {
                                ...
                                if (rUnion.ok) {
                                    ...
                                    const aU = jU?.aliases && typeof jU.aliases === "object"
                                        ? jU.aliases
                                        : {};
                                    ...
                                    try {
                                        ...
                                        const filtered = computeActivePins(itemsFiltered, activeIds);
    ...
    const handleScan = useCallback(async (raw) => {
        ...
        if (normalized === lastHandledScanRef.current &&
        ...
        if (isScanningRef.current || scanInFlightRef.current)
            return; // avoid overlapping flows
    ...
    useEffect(() => {
        ...
        if (want && seen && !pathsEqual(seen, want))
            return; // ignore scans from other scanner paths
    ...
    useEffect(() => {
        ...
        const lastAt = serial.lastScanAt;
        const sseOk = !!serial.sseConnected;
        const stale = !(typeof lastAt === "number" && isFinite(lastAt)) ||
            Date.now() - lastAt > STALE_MS;
        if (sseOk && !stale)
            return; // healthy SSE path — skip polling
        let stopped = false;
        let lastPollAt = 0;
                ...
                if (res.ok) {
                    ...
                    try {
                        if (typeof retryInMs === "number")
                            window.__scannerRetry = retryInMs;
                    }
                    catch { }
                    const raw = typeof code === "string" ? code.trim() : "";
                    if (raw) {
                        if (path && !isAcmPath(path))
                            return;
                        ...
                        const isNotPresent = lower.includes("scanner port not present") ||
    ...
    const handleManualSubmit = (submittedNumber) => {
    ...
    const appCurrentViewType = mainView === "settingsConfiguration" || mainView === "settingsBranches"
        ? "settings"
        : "main";
    ...
    const showBranchesSettings = (id) => {
        if (id != null)
            setCurrentConfigIdForProgram(id);
    ...
    return (<div className="relative flex min-h-screen bg-slate-100 dark:bg-slate-900">
      {mainView === "dashboard" && (<BranchControlSidebar isOpen={isLeftSidebarOpen} toggleSidebar={toggleLeftSidebar} branches={branchesData} onSetStatus={(id, status) => setBranchesData((data) => data.map((b) => (b.id === id ? { ...b, testStatus: status } : b)))} sidebarWidthProvided={SIDEBAR_WIDTH} appHeaderHeight={actualHeaderHeight}/>)}

      <div className="flex flex-1 flex-col transition-all" style={{ marginLeft: leftOffset }}>
        {mainView === "dashboard" && (<Header onSettingsClick={handleHeaderClick} currentView={appCurrentViewType} isSidebarOpen={isLeftSidebarOpen} onToggleSidebar={toggleLeftSidebar}/>)}

        <main className="flex-1 overflow-auto bg-gray-50 dark:bg-slate-900">
          {mainView === "dashboard" ? (<>
              {desiredTail && (<div className="px-2 pt-0 flex flex-wrap gap-2">
                  ...
                  {desiredTail &&
                    (() => {
                        ...
                        return (<span className={`${badgeBase} ${badgeColor}`}>
                          Scanner: {desiredTail}
                  ...
                  {(() => {
                    const mac = (macAddress || "").toUpperCase();
                    const on = !!(serial.sseConnected && mac);
                    const cnt = Number(serial.evCount || 0);
                    const badgeBase = "inline-flex items-center gap-1 rounded-full px-3 py-1 text-[12px] md:text-[13px] font-extrabold";
                    const badgeColor = on
                        ? "border border-emerald-300 bg-emerald-50 text-emerald-900 dark:bg-emerald-900/20 dark:text-emerald-200"
                        : "border border-slate-300 bg-slate-50 text-slate-700 dark:bg-slate-800/40 dark:text-slate-200";
                    return (<span className={`${badgeBase} ${badgeColor}`} title={mac ? `MAC ${mac}` : "inactive"}>
                        Live:
              ...
              {(() => {
                ...
                return (<BranchDashboardMainContent key={session} appHeaderHeight={actualHeaderHeight} onManualSubmit={handleManualSubmit} onScanAgainRequest={() => loadBranchesData()} branchesData={effBranches} groupedBranches={effGroups} checkFailures={effFailures} nameHints={nameHints} kfbNumber={kfbNumber} kfbInfo={kfbInfo} isScanning={isScanning && showScanUi} macAddress={macAddress} activeKssks={effActiveKssks} lastEv={suppressLive ? null : serial.lastEv} lastEvTick={suppressLive ? 0 : serial.lastEvTick} normalPins={suppressLive ? undefined : effNormalPins} latchPins={suppressLive ? undefined : effLatchPins} onResetKfb={handleResetKfb} onFinalizeOk={finalizeOkForMac} flashOkTick={okFlashTick} okSystemNote={okSystemNote} disableOkAnimation={disableOkAnimation}/>);
            })()}

              <form onSubmit={handleKfbSubmit} className="hidden"/>
            </>) : mainView === "settingsConfiguration" ? (<SettingsPageContent onNavigateBack={showDashboard} onShowProgramForConfig={showBranchesSettings}/>) : (<SettingsBranchesPageContent onNavigateBack={showDashboard} configId={currentConfigIdForProgram}/>)}
        </main>
      </div>

      <SettingsRightSidebar isOpen={isSettingsSidebarOpen} onClose={() => setIsSettingsSidebarOpen(false)} appHeaderHeight={actualHeaderHeight} onShowConfigurationInMain={showConfig} onShowBranchesSettingsInMain={() => showBranchesSettings()}/>
      ...
      <AnimatePresence>
        {overlay.open && (<m.div variants={bg} initial="hidden" animate="visible" exit="exit" style={{
                position: "fixed",
                inset: 0,
                ...
                placeItems: "center",
                zIndex: 9999,
            }} aria-live="assertive" aria-label={overlay.kind === "scanning" && overlay.code
                ? overlay.code
            ...
            <m.div variants={card} initial="hidden" animate="visible" exit="exit" style={{ display: "grid", justifyItems: "center", gap: 8 }}>
              {overlay.kind === "success" ? (<>
                  <m.div initial={{ scale: reduce ? 1 : 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 260, damping: 20 }}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{
                    width: 160,
                    height: 160,
                  ...
                  {(() => {
                    ...
                    const sanitizeErrorText = (t) => {
                        const keep = new Set(["NOTHING TO CHECK HERE"]);
                        if (keep.has(t))
                            return t;
                        const tooLong = t.length > 48;
                        if (/RESULT|\u2190|reply\s+from|FAIL|MISSING/i.test(t) ||
                            tooLong)
                            return "ERROR";
                        return t;
                    };
                    const bigText = isScanningWithCode
                        ...
                        : isErrorWithCode
                            ...
                            : overlay.kind.toUpperCase();
                    return (<m.div variants={heading} style={{
                            ...
                            textAlign: "center",
                            fontFamily: overlay.kind === "scanning"
                  ...
                  {overlay.kind === "scanning" && overlay.code ? (<m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: reduce ? 0 : 0.05 }} style={{ fontSize: 18, color: "#f1f5f9", opacity: 0.95 }}>
                      SCANNING…
                    </m.div>) : overlay.kind === "error" && overlay.code ? (<m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: reduce ? 0 : 0.05 }} style={{
                        fontSize: 18,
                        color: "#f1f5f9",
                        opacity: 0.95,
                        textAlign: "center",
                      ...
                      ERROR
                    </m.div>) : overlay.code ? (<m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: reduce ? 0 : 0.05 }} style={{
                        fontSize: 16,
                        color: "#f1f5f9",
                        opacity: 0.95,
                        wordBreak: "break-all",
