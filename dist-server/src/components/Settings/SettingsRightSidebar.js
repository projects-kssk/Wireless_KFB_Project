...
/* ────────────────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────────────────── */
const SheetGlass = 'bg-[linear-gradient(145deg,rgba(255,255,255,0.92),rgba(246,248,255,0.78))] ' +
...
const TileGlass = 'bg-[linear-gradient(180deg,rgba(255,255,255,0.9),rgba(245,247,255,0.76))] ' +
    'dark:bg-[linear-gradient(180deg,rgba(30,41,59,0.8),rgba(30,41,59,0.64))] ' +
    'backdrop-blur-xl border border-white/60 dark:border-white/10 ' +
    'shadow-[0_12px_36px_-10px_rgba(2,6,23,0.40)]';
const Aura = 'pointer-events-none absolute inset-0 rounded-[36px] ' +
...
/* ────────────────────────────────────────────────────────────────────────────
   Component
   ──────────────────────────────────────────────────────────────────────────── */
export default function SettingsRightSidebar({ isOpen, onClose, appHeaderHeight: _appHeaderHeight, // API compatibility
onShowConfigurationInMain, onShowBranchesSettingsInMain, }) {
    if (!isOpen)
        return null;
    const otpLength = appConfig?.otpLength ?? 4;
    const [activeSection, setActiveSection] = useState('MAIN_S');
    // Locks
    const [locks, setLocks] = useState({ MAIN_S: null, CONFIG_S: null, BRANCHES_S: null });
    ...
    const scheduleNextExpiry = useCallback((map) => {
        if (expiryTimerRef.current) {
            clearTimeout(expiryTimerRef.current);
            expiryTimerRef.current = null;
        }
        const now = Date.now();
        const upcoming = Object.values(map).filter(Boolean).map((t) => Math.max(0, t - now));
    ...
    const lockUntil = (id) => locks[id];
    const goToPin = (target) => {
        if (isLocked(target))
            return;
        setPinTarget(target);
        setAttemptsLeft(3);
    ...
    return (<>
      ...
      <div className="fixed inset-0 z-[70] bg-slate-950/55" onClick={onClose} aria-hidden/>

      {/* Static sheet: no entrance animation */}
      <section role="dialog" aria-modal="true" aria-label="Settings" className="fixed inset-0 z-[75] flex items-center justify-center p-4 md:p-8">
        <div className={['relative w-full max-w-[1640px]', SheetGlass, 'rounded-[36px] p-6 md:p-8 lg:p-10'].join(' ')}>
          ...
          <div className="flex items-end justify-between gap-4 pb-5 border-b border-white/60 dark:border-white/10">
            ...
            <div className="flex gap-3">
              {pinMode && (<button onClick={() => { setPinMode(false); setPinTarget(null); setPinError(''); }} className="inline-flex h-11 md:h-12 px-5 items-center justify-center rounded-full bg-white/85 dark:bg-slate-800/70 border border-white/60 dark:border-white/10 shadow text-sm font-semibold text-slate-900 dark:text-slate-100">
                  Back
                </button>)}
              <button onClick={onClose} className="inline-flex h-11 md:h-12 w-11 md:w-12 items-center justify-center rounded-full bg-white/92 dark:bg-slate-800/70 border border-white/60 dark:border-white/10 shadow" aria-label="Close settings">
          ...
          <div className="mt-6 md:mt-8">
            {!pinMode && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                <Tile active={activeSection === 'MAIN_S'} label="Main" accent="sky" Icon={SettingsHomeIcon} badgeNum={1} locked={isLocked('MAIN_S')} lockUntil={lockUntil('MAIN_S')} onClick={() => handleTileClick('MAIN_S')}/>
                <Tile active={activeSection === 'CONFIG_S'} label="KFB Config" accent="violet" Icon={SettingsCogIcon} badgeNum={2} locked={isLocked('CONFIG_S')} lockUntil={lockUntil('CONFIG_S')} onClick={() => handleTileClick('CONFIG_S')}/>
                <Tile active={activeSection === 'BRANCHES_S'} label="Program" accent="emerald" Icon={SettingsCubeIcon} badgeNum={3} locked={isLocked('BRANCHES_S')} lockUntil={lockUntil('BRANCHES_S')} onClick={() => handleTileClick('BRANCHES_S')}/>
              </div>)}

            {pinMode && pinTarget && (<PinForm otpLength={otpLength} sectionLabel={pinTarget === 'CONFIG_S' ? 'KFB CONFIG' : 'PROGRAM'} attemptsLeft={attemptsLeft} errorMessage={pinError} onSubmit={handlePinSubmit}/>)}
          </div>
        </div>
      </section>
    </>);
...
const Tile = React.memo(({ label, active, onClick, Icon, accent, badgeNum, locked, lockUntil }) => {
    const acc = accentMap[accent];
    const disabled = !!locked;
    return (<button onClick={() => !disabled && onClick()} className={[
        ...
        ].join(' ')} aria-pressed={active} aria-disabled={disabled}>
        {/* Step badge */}
        <span className={[
        ...
        <span aria-hidden className={`pointer-events-none absolute inset-0 rounded-[28px] opacity-0 ${acc.borderGlow} transition-opacity duration-200 group-hover:opacity-100`}/>

        {/* LOCKED overlay */}
        {locked && (<>
            <div className="absolute inset-0 z-10 rounded-[28px] pointer-events-none" style={{
                background: 'linear-gradient(180deg,rgba(2,6,23,0.20),rgba(2,6,23,0.30)), radial-gradient(100% 140% at 50% 20%, rgba(255,255,255,0.05), rgba(255,255,255,0) 60%)',
                backdropFilter: 'blur(1.5px)',
            }}/>
            <div className="absolute inset-0 z-[11] rounded-[28px] pointer-events-none mix-blend-overlay" style={{
                backgroundImage: 'repeating-linear-gradient(135deg, rgba(255,255,255,0.08) 0px, rgba(255,255,255,0.08) 12px, rgba(255,255,255,0) 12px, rgba(255,255,255,0) 24px)',
            }}/>
          </>)}
        ...
        <div className="relative z-[12] flex h-full flex-col items-center justify-center gap-8 lg:gap-10">
          ...
          <div className={`relative z-[20] text-center ${locked ? 'blur-[1px] opacity-85' : ''}`}>
            ...
            {active && (<div className="mt-3 inline-flex items-center rounded-full px-4 py-1.5 text-base font-semibold bg-black/5 dark:bg-white/10 text-slate-900 dark:text-slate-100">
                Selected
              </div>)}
          </div>
        </div>
        ...
        {locked && (<div className="absolute z-[22] left-1/2 -translate-x-1/2 top-6 pointer-events-none">
...
const LockGlyph = ({ className }) => (<svg viewBox="0 0 24 24" className={className} aria-hidden>
    <path d="M12 1.75a5.25 5.25 0 00-5.25 5.25v2.25H5.5A2.75 2.75 0 002.75 12v7.25A2.75 2.75 0 005.5 22h13a2.75 2.75 0 002.75-2.75V12A2.75 2.75 0 0018.5 9.25h-1.25V7A5.25 5.25 0 0012 1.75zm-3.75 7.5V7A3.75 3.75 0 0112 3.25 3.75 3.75 0 0115.75 7v2.25h-7.5z" fill="currentColor"/>
  </svg>);
/* Re-renders only itself every second */
const CountdownLabel = React.memo(({ until }) => {
...
const PinForm = React.memo(({ otpLength, sectionLabel, attemptsLeft, errorMessage, onSubmit }) => {
    const inputsRef = useRef([]);
    const [values, setValues] = useState(() => Array.from({ length: otpLength }, () => ''));
    useEffect(() => {
        inputsRef.current[0]?.focus();
    }, []);
    const codeWith = (nextValues) => nextValues.join('');
    const setChar = (index, val) => {
    ...
    const onPaste = (e) => {
        ...
        setValues((prev) => {
            ...
            if (next.every(Boolean))
                setTimeout(() => onSubmit(codeWith(next)), 30);
            else
                inputsRef.current[Math.min(chars.length, otpLength - 1)]?.focus();
            return next;
        });
    };
    return (<div className="flex flex-col items-center text-center">
        ...
        <div className="mt-6 md:mt-8 flex gap-4 md:gap-5">
          {Array.from({ length: otpLength }).map((_, i) => (<div key={i} className="relative">
              <input ref={(el) => { inputsRef.current[i] = el; }} value={values[i]} onChange={(e) => setChar(i, e.target.value)} onKeyDown={(e) => onKeyDown(i, e)} onPaste={onPaste} type="password" inputMode="numeric" pattern="[0-9]*" autoComplete="one-time-code" autoCorrect="off" autoCapitalize="none" name={`otp-digit-${i}`} data-lpignore="true" data-1p-ignore="" className={[
            ...
            ].join(' ')} aria-label={`PIN digit ${i + 1}`}/>
              <span className="pointer-events-none absolute inset-0 rounded-2xl md:rounded-3xl ring-1 ring-white/60 dark:ring-white/10"/>
            </div>))}
        </div>

        {errorMessage && (<div className="mt-4 text-lg md:text-xl font-semibold text-rose-600 dark:text-rose-400">{errorMessage}</div>)}
      </div>);
});
