...
async function ensureLogDir(dir = LOG_DIR) { try {
    await fs.mkdir(dir, { recursive: true });
}
catch { } }
async function pruneOldMonitorLogs(root, maxAgeDays = 31) {
    try {
        const now = Date.now();
...
async function appendLog(entry) {
    ...
    try {
        ...
        const mac = entry?.mac;
        const kssk = entry?.kssk;
        if (evt === 'monitor.send') {
            const payload = entry?.sent ?? entry?.built; // ← prefer 'sent'
        ...
        if (evt === 'monitor.success') {
            if (startOnly)
                return; // suppress OK lines in start-only mode
...
function pinDiff(requested = [], built = []) {
...
async function loadSerial() {
    ...
    if (typeof sendAndReceive !== "function")
        throw new Error("sendAndReceive missing");
    if (typeof sendToEsp !== "function")
        throw new Error("sendToEsp missing");
    return { sendAndReceive, sendToEsp };
}
/* ----------------- pin extraction ----------------- */
function parseObjPos(objPos) {
    const parts = String(objPos || "").split(",").map(s => s.trim());
    // Policy: do NOT derive a pin when no comma is present
    if (parts.length < 2)
        return { pin: null, latch: false };
    let latch = false;
...
function extractPinsFromSequence(seq, mac) {
    ...
    for (const s of seq) {
        ...
        if (s.objGroup) {
            ...
            if (m && wantMac && m[1].toUpperCase() !== wantMac)
                continue;
        }
...
export async function GET(req) {
    try {
        ...
        try {
            resp.headers.set('X-Req-Id', ridFrom(req));
        }
        catch { }
        return resp;
    }
...
export async function POST(request) {
    ...
    const cmds = [];
    if (normalPins.length)
        cmds.push({ cmd: `MONITOR ${normalPins.join(',')} ${mac}`, kind: 'normal' });
    if (latchPins.length)
        cmds.push({ cmd: `MONITOR LATCH ${latchPins.length} ${latchPins.join(',')} ${mac}`, kind: 'latch' });
    ...
    const mismatch = (diffs.normal.missing.length + diffs.normal.added.length +
    ...
    try {
        if (kssk) {
            ...
            if (!names || Object.keys(names).length === 0) {
                try {
                    ...
                    if (rawU) {
                        ...
                        const nU = (dU?.names && typeof dU.names === 'object') ? dU.names : {};
                        // only take names for pins we are sending
                        const need = new Set([...normalPins, ...latchPins]);
                        const picked = {};
                        for (const [pin, label] of Object.entries(nU)) {
                            const p = Number(pin);
                            if (Number.isFinite(p) && need.has(p))
                                picked[pin] = String(label);
                        }
                        names = picked;
                    }
                }
            ...
            try {
                await r.set(keyK, value);
            }
            catch { }
            try {
                await r.sadd(`kfb:aliases:index:${macUp}`, String(kssk));
            }
            catch { }
            // Also record "last pins used" snapshot for this KSK to support watcher fallbacks
            try {
                await r.set(`kfb:lastpins:${macUp}:${String(kssk)}`, JSON.stringify({ normalPins, latchPins, ts: tsNow }));
            }
            catch { }
        }
    }
