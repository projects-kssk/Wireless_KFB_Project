...
export async function POST(req: Request) {
  try {
    ...
    const { port, parser } = getEspLineStream() as any;
    if (!port?.write)
    ...
    await new Promise<void>((resolve, reject) => {
      port.write(cmd, (err: any) => (err ? reject(err) : resolve()));
    });

    // Optionally listen briefly for READY/OK to give immediate feedback
    const ready = await new Promise<boolean>((resolve) => {
      let done = false;
      const cleanup = (fn?: any) => {
        try { parser?.off?.('data', fn); } catch {}
      };
      const onData = (b: Buffer | string) => {
        const line = String(b).trim().toUpperCase();
        if (/^(READY|OK)\b/.test(line)) {
          if (!done) { done = true; cleanup(onData); clearTimeout(t); resolve(true); }
        }
      };
      const t = setTimeout(() => {
        if (!done) { done = true; cleanup(onData); resolve(false); }
      }, 2500);
