...
const XMarkIcon = (props) => (<svg width={24} height={24} fill="none" viewBox="0 0 24 24" {...props}>
    <path d="M6 6l12 12M6 18L18 6" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"/>
  </svg>);
export const FlexibleOtpModal = ({ isOpen, onClose, onSubmit, attemptsLeft, errorMessage, otpLength, title = "PIN CODE", promptText, }) => {
    ...
    const inputRefs = useRef(Array(otpLength).fill(null));
    //
    ...
    return (<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 font-sans">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md sm:max-w-lg md:max-w-xl flex flex-col items-center">
        <div className="w-full flex justify-end mb-2">
          <button onClick={onClose} className="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 p-2 rounded-full focus:outline-none focus:ring-2 focus:ring-sky-500" aria-label="Close OTP modal" type="button">
        ...
        {promptText && (<p className="text-md sm:text-lg text-slate-600 dark:text-slate-400 mb-6 text-center">
            {promptText}
          </p>)}
        ...
        <form onSubmit={handleFormSubmit} className="w-full flex flex-col items-center">
          ...
          <div className="flex justify-center space-x-2 sm:space-x-3 md:space-x-4 mb-8 w-full" onPaste={handlePaste}>
            {otpDigits.map((digit, idx) => (<input key={idx} ref={(el) => {
                inputRefs.current[idx] = el;
            }} type="password" // <–– Change here
         value={digit} onChange={(e) => handleChange(idx, e.target.value)} onKeyDown={(e) => handleKeyDown(idx, e)} maxLength={1} className="w-14 h-16 sm:w-16 sm:h-20 md:w-20 md:h-24 text-center text-3xl sm:text-4xl md:text-5xl font-semibold bg-slate-100 dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500 caret-sky-500" inputMode="numeric" // keep numeric keypad
         pattern="[0-9]*" autoComplete="one-time-code" aria-label={`OTP digit ${idx + 1}`}/>))}
          </div>

          {errorMessage && (<p className="text-lg sm:text-xl text-red-500 dark:text-red-400 mt-4 mb-2 text-center min-h-[1.5rem] sm:min-h-[1.75rem]">
              {errorMessage}
            </p>)}

          <button type="submit" className="w-full mt-4 sm:mt-6 px-6 py-3 bg-sky-600 text-white text-lg sm:text-xl font-semibold rounded-lg hover:bg-sky-700 focus:outline-none focus:ring-4 focus:ring-sky-500 focus:ring-opacity-50 transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed" disabled={otpDigits.includes("") || otpDigits.join("").length !== otpLength}>
            Submit
          </button>
        </form>
      </div>
