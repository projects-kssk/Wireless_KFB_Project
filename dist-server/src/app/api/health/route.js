...
function parseHostPort(raw, defPort = 10080) {
    if (!raw)
...
async function probeTcp(host, port, timeoutMs = 1500) {
    return new Promise((resolve) => {
        ...
        let done = false;
        const finish = (ok, error) => { if (done)
            return; done = true; try {
            s.destroy();
        }
        catch { } resolve({ ok, used, error }); };
        s.setNoDelay(true);
        s.setTimeout(timeoutMs);
        s.once('connect', () => finish(true));
        s.once('timeout', () => finish(false, 'timeout'));
        s.once('error', (e) => finish(false, e?.message || 'tcp error'));
        try {
            s.connect(port, host);
        }
        catch (e) {
            finish(false, e?.message || 'connect error');
        }
    });
}
export async function GET() {
    ...
    let krosy = { onlineConfigured: online, ok: null };
    if (online) {
        ...
        if (host && port) {
            try {
                const res = await probeTcp(host, port, Number(process.env.KROSY_TCP_TIMEOUT_MS || 1500));
                krosy = { onlineConfigured: online, ok: res.ok, used: res.used, error: res.error };
            }
            catch {
                krosy = { onlineConfigured: online, ok: false, used: `tcp://${host}:${port}`, error: 'probe failed' };
            }
        }
        else {
    ...
    try {
        serial.present = await isEspPresent();
    }
    catch { }
    ...
    let stationLocks = { stationId: stationId || null, active: null };
    if (redis.ready && stationId) {
        try {
            const r = getRedis();
            const key = `ksk:station:${stationId}`;
            const members = await r.smembers?.(key)?.catch?.(() => []) ?? [];
            stationLocks.active = Array.isArray(members) ? members.length : 0;
        }
        catch {
            stationLocks.active = null;
        }
    }
