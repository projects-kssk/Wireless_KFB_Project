...
const BASE_W = 1440;
const BASE_H = 900;
export default function ViewportScaler({ children }) {
    ...
    const getQueryMode = () => {
        ...
        return m === 'tv' || m === 'pc' || m === 'auto' ? m : null;
    };
    const getStoredMode = () => null;
    const isTVLike = () => {
        if (!canUseDOM)
            return false;
    ...
    const resolveMode = (m) => (m === 'auto' ? (isTVLike() ? 'tv' : 'pc') : m);
    // initial mode
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            window.removeEventListener('orientationchange', debouncedCalc);
            window.visualViewport?.removeEventListener('resize', debouncedCalc);
        };
    }, [mode]);
    ...
    return activeTV ? (<div style={wrapperStyle}>
