...
/* ────────────────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────────────────── */
type SettingsIconProps = { size?: number | string; className?: string; title?: string };
...
function StatusStrip({ tone, text }: { tone: 'indigo' | 'emerald' | 'sky' | 'slate'; text: string }) {
  ...
  return (
    <div className={['relative mt-4 w-full overflow-hidden select-none', 'rounded-[22px] bg-white/85 dark:bg-[#121212]/85', 'backdrop-blur-xl ring-1', toneMap.ring, 'shadow-[0_8px_28px_rgba(2,6,23,.10)]'].join(' ')}>
...
function SimpleLinkAnimation({ searching, success }: { searching: boolean; success: boolean }) {
...
function StatusBanner({ status, mac, error }: { status: DiscoverState; mac: string | null; error: string | null }) {
...
function DiscoverEspModal({
  ...
  const stripTone: 'indigo' | 'emerald' | 'sky' | 'slate' =
    status === 'searching' ? 'indigo' : showSuccess ? 'emerald' : status === 'success' ? 'sky' : 'slate';
...
const EspDiscoverButton: React.FC<{ onClick: () => void; busy: boolean; mac?: string | null }> = ({ onClick, busy, mac }) => (
  <m.button
    type="button"
    onClick={onClick}
    whileHover={{ y: -2 }}
