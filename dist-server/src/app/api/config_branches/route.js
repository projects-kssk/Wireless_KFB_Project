...
export async function GET(request) {
    ...
    if (!cfgIdS || !detailS) {
        return NextResponse.json({ error: 'Missing ?configId=… & ?detailId=…' }, { status: 400 });
    }
    const configId = Number(cfgIdS);
    const detailId = Number(detailS);
    if (Number.isNaN(configId) || Number.isNaN(detailId)) {
        return NextResponse.json({ error: 'configId/detailId must be numbers' }, { status: 400 });
    }
...
export async function DELETE(request) {
    ...
    if (!detailS || !branchS) {
        return NextResponse.json({ error: 'Missing ?detailId=… & ?branchId=…' }, { status: 400 });
    }
    const detailId = Number(detailS);
    const branchId = Number(branchS);
    if (Number.isNaN(detailId) || Number.isNaN(branchId)) {
        return NextResponse.json({ error: 'detailId/branchId must be numbers' }, { status: 400 });
    }
    const client = await pool.connect();
    try {
        await client.query(`
