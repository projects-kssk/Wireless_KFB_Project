...
export async function GET(request) {
    ...
    try {
        ...
        if (idsRaw) {
            ...
            const { rows } = await pool.query(`SELECT b.id, b.name, cb.not_tested, cb.loose_contact
          FROM branches b
...
export async function POST(request) {
    ...
    try {
        ...
        await client.query(`INSERT INTO branches(name)
         VALUES ($1)
         ON CONFLICT (name) DO NOTHING`, [name]);
        // 3) fetch *that* branch row (new or existing)
        const sel = await client.query(`SELECT id, name
         FROM branches
        WHERE name = $1
        LIMIT 1`, [name]);
