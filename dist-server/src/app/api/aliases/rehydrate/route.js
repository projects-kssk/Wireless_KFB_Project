...
export async function POST(req) {
    try {
        ...
        try {
            ...
            log.error('rehydrate: scan failed', { error: String(e?.message ?? e) });
        }
        const unionNames = {};
        const nSet = new Set();
        const lSet = new Set();
        ...
        if (ids.length)
            await r.sadd(`kfb:aliases:index:${mac}`, ...ids).catch(() => { });
        log.info('POST aliases rehydrate', { mac, items: ids.length, normal: out.normalPins.length, latch: out.latchPins.length });
        // Notify SSE subscribers
        try {
            broadcast({ type: 'aliases/union', mac, names: out.names, normalPins: out.normalPins, latchPins: out.latchPins });
        }
        catch { }
