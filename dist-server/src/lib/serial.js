...
function armEsp() {
    ...
    parser.on("data", (buf) => {
        ...
        if (!s)
            return;
        if (process.env.ESP_DEBUG)
            LOG.tag('esp').debug(`[${path}] ${s}`);
        ring.push(s);
        ringIds.push(nextId++);
        if (ring.length > 400) {
            ring.shift();
...
export function getEspLineStream() { return armEsp(); }
export function mark() {
    const { ringIds } = armEsp();
    return ringIds.length ? ringIds[ringIds.length - 1] : 0;
}
export function flushHistory() { return mark(); }
// --- convenience accessor
export function getEspActivity() {
    const s = getEspLineStream();
    if (!s.lastSeenAt)
        return { lastSeenAt: null, ageMs: null, lastLine: undefined };
    const ageMs = Date.now() - s.lastSeenAt;
    return { lastSeenAt: s.lastSeenAt, ageMs, lastLine: s.lastLine };
}
async function waitUntilOpen(port) {
    if (port.isOpen)
        return;
...
async function writeLine(line) {
    ...
    await new Promise((resolve, reject) => port.write(out, (err) => {
        if (err)
            return reject(err);
        port.drain((derr) => (derr ? reject(derr) : resolve()));
    }));
    if (process.env.ESP_DEBUG)
        LOG.tag('esp').debug(`⟶ [you] ${line}`);
}
export function waitForLine(matcher, signal, timeoutMs = 12_000, opts = {}) {
    const { ring, ringIds, subs } = armEsp();
    const isMatch = typeof matcher === "function" ? matcher : (s) => matcher.test(s);
    const since = opts.since ?? undefined;
    return new Promise((resolve, reject) => {
        for (let i = ring.length - 1; i >= 0; i--) {
            if (since && ringIds[i] <= since)
                break;
            const s = ring[i];
            if (isMatch(s))
                return resolve(s);
        ...
        const onLine = (s, id) => { if (since && id <= since)
            return; if (isMatch(s))
            finish(undefined, s); };
...
export function waitForNextLine(matcher, signal, timeoutMs = 12_000) {
    return waitForLine(matcher, signal, timeoutMs, { since: mark() });
}
export async function sendToEsp(cmd) { await writeLine(cmd); }
export async function sendAndReceive(cmd, timeout = 10_000) {
    const RX_OK_STRICT = /^\s*(?:>>\s*)?(?:RESULT\s+)?(?:SUCCESS|OK)\s*$/i;
    const matcher = (line) => {
        const up = line.toUpperCase();
        if (up.startsWith("ERROR"))
            return true;
        if (/\bFAIL(?:URE|URES)\b/.test(up))
            return true;
...
export async function pingEsp() {
    const tmpl = (process.env.ESP_PING_CMD ?? "PING").trim();
    if (!tmpl)
        return { ok: true, raw: "present (no ping)" };
...
export async function espHealth(opts) {
    const probeEnv = (process.env.ESP_HEALTH_PROBE ?? "never").toLowerCase();
    ...
    if (!present)
        return { present, ok: false, raw: "not present" };
...
const GG = globalThis;
if (!GG.__scanners)
    GG.__scanners = new Map();
const scanners = GG.__scanners;
const now = () => Date.now();
const inCooldown = (r) => now() < r.nextAttemptAt;
const bumpCooldown = (r, ms) => {
    r.retryMs = Math.min(ms ?? Math.ceil(r.retryMs * 1.7), 60_000);
    r.nextAttemptAt = now() + r.retryMs;
};
const resetCooldown = (r) => { r.retryMs = 2000; r.nextAttemptAt = 0; };
function broadcastScannerErrorOnce(r, msg, path) {
    const t = now();
...
function attachScannerHandlers(path, state, retry, port, parser) {
    parser.on("data", (raw) => {
...
export async function ensureScannerForPath(path, baudRate = 115200) {
    ...
    if (!ALLOW_USB_SCANNER && /\/ttyUSB\d+$/i.test(path)) {
        ...
        if (LOG_SKIPS) {
            if (espPath && path === espPath)
                LOG.tag('scanner').info(`skip scanner on ESP path ${path}`);
            else
                LOG.tag('scanner').warn(`skip scanner open on USB path ${path} (set ALLOW_USB_SCANNER=1 to allow)`);
        }
    ...
    if (inCooldown(retry))
        throw new Error(`SCANNER_COOLDOWN ${path} until ${new Date(retry.nextAttemptAt).toISOString()}`);
    function looksLikeSamePath(candidate, wanted) {
        if (candidate === wanted)
            return true;
...
export function considerDevicesForScanner(devices, pathsInput = process.env.SCANNER_TTY_PATHS ?? process.env.SCANNER_TTY_PATH ?? "/dev/ttyACM0") {
...
if (!__g.__serial_sig_handlers_installed) {
    try {
        process.setMaxListeners?.(Math.max(20, process.getMaxListeners?.() ?? 10));
    }
    catch { }
