...
const FORCE_RESULT = ["ok", "true", "1", "yes"].includes(FORCE_RESULT_RAW) ? true :
...
/* ===== TCP ===== */
function terminatorAppend(xml) {
    if (TCP_TERMINATOR === "newline")
        return xml + "\n";
    if (TCP_TERMINATOR === "crlf")
        return xml + "\r\n";
    if (TCP_TERMINATOR === "null")
        return xml + "\0";
    return xml;
}
const xmlDone = (s) => s.trim().toLowerCase().endsWith("</krosy>");
const isAckToken = (s) => {
...
function apikingResultFromWorkingData(workingDataXml, overrides, opts // NEW
) {
    ...
    if (!workingData)
        throw new Error("No <workingData> in payload");
    ...
    if (sequencer) {
        ...
        for (const seg of segments) {
            ...
            for (const seq of sequences) {
                ...
                const isTarget = compType === "clip" && measType === "default" &&
...
export async function POST(req) {
    ...
    const month = `${yyyy}-${mm}`;
    let idSan = (String(body?.intksk || '')).replace(/[^0-9A-Za-z_-]/g, '').slice(-12) || '';
    if (!idSan && typeof body?.workingDataXml === 'string') {
        const xml = String(body.workingDataXml);
        try {
    ...
    const bodyForce = ["ok", "true", "1", "yes"].includes(bodyForceRaw) ? true :
    ...
    let meta;
    try {
        const built = apikingResultFromWorkingData(workingDataXml, { requestID: reqId }, { forceResult: bodyForce ?? FORCE_RESULT } // <-- pass it here
        );
        resultXml = built.xml;
        meta = built.meta;
    }
        ...
        return new Response(JSON.stringify({ ok: false, phase: "apikingResult", error: e?.message || String(e) }, null, 2), { status: 400, headers: { "Content-Type": "application/json", ...cors(req) } });
    }
