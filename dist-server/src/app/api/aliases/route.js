...
function keyFor(mac) { return `kfb:aliases:${mac.toUpperCase()}`; }
function keyForKsk(mac, ksk) { return `kfb:aliases:${mac.toUpperCase()}:${ksk}`; }
function indexKey(mac) { return `kfb:aliases:index:${mac.toUpperCase()}`; }
async function connectIfNeeded(r, timeoutMs = 600) {
    if (!r)
        return false;
...
async function withRetry(fn, tries = 3, delayMs = 150) {
    let last = null;
...
export async function GET(req) {
    try {
        ...
        if (all) {
            ...
            try {
                if (typeof r.status === 'string' && r.status !== 'ready')
                    await (r.connect?.().catch(() => { }));
            }
            catch { }
            // Return all KSK-specific alias bundles we know for this MAC
            let members = await r.smembers(indexKey(macRaw)).catch(() => []);
            ...
            const rows = await Promise.all(members.map(async (ksk) => {
                try {
        ...
        const raw = await r.get(keyFor(macRaw)).catch(() => null);
        if (!raw)
            return NextResponse.json({ aliases: {}, normalPins: [], latchPins: [] });
        let data = {};
        try {
            data = JSON.parse(raw);
        }
...
export async function POST(req) {
    try {
        ...
        if (!MAC_RE.test(mac))
            return NextResponse.json({ error: 'invalid-mac' }, { status: 400 });
        let aliases = body?.aliases && typeof body.aliases === 'object' ? body.aliases : {};
        let normalPins = Array.isArray(body?.normalPins) ? body.normalPins : [];
        let latchPins = Array.isArray(body?.latchPins) ? body.latchPins : [];
        const ksk = (body?.ksk ? String(body.ksk) : (body?.kssk ? String(body.kssk) : '')).trim();
        ...
        if (xml) {
            try {
                ...
                const ex = (() => {
                    ...
                    try {
                        ...
                        while ((m = re.exec(xml))) {
                            ...
                            if (!ogMac || ogMac === '00:00:00:00:00:00' || ogMac !== mac)
                                continue;
                ...
                if (ex.normalPins.length || ex.latchPins.length) {
                    ...
                    if (ex.names && Object.keys(ex.names).length)
                        aliases = ex.names;
                }
            }
            catch { }
        ...
        let wroteMac = false;
        try {
            await withRetry(() => r.set(keyFor(mac), value));
            wroteMac = true;
        }
        catch (e) {
            log.error('POST aliases set mac failed', { mac, error: String(e?.message ?? e) });
        }
        if (ksk) {
            try {
                await withRetry(() => r.set(keyForKsk(mac, ksk), value));
            }
            catch (e) {
                log.error('POST aliases set ksk failed', { mac, ksk, error: String(e?.message ?? e) });
            }
            try {
                await withRetry(() => r.sadd(indexKey(mac), ksk));
            }
            catch (e) {
                log.error('POST aliases index sadd failed', { mac, ksk, error: String(e?.message ?? e) });
            }
            if (xml) {
                try {
        ...
        if (!wroteMac)
            return NextResponse.json({ error: 'redis_write_failed' }, { status: 503 });
        ...
        try {
            ...
            const curMembers = await r.smembers(indexKey(mac)).catch(() => []);
            let foundIds = [];
            try {
            ...
            if (toAdd.length) {
                try {
                    await r.sadd(indexKey(mac), ...toAdd);
                    log.info('POST aliases rehydrate index', { mac, added: toAdd.length });
                }
                catch { }
            }
            ...
            for (const id of members) {
                try {
                    ...
                    if (d?.hints && typeof d.hints === 'object') {
                        for (const [pin, name] of Object.entries(d.hints)) {
                            if (!unionHints[pin])
                                unionHints[pin] = name;
                        }
                    }
                    if (Array.isArray(d?.normalPins))
                        allN = Array.from(new Set([...allN, ...d.normalPins]));
                    if (Array.isArray(d?.latchPins))
                        allL = Array.from(new Set([...allL, ...d.latchPins]));
                }
                catch { }
            }
            ...
            try {
                await withRetry(() => r.set(keyFor(mac), unionVal));
            }
            catch (e) {
                log.error('POST aliases union set failed', { mac, error: String(e?.message ?? e) });
            }
            log.info('POST aliases union rebuilt', { mac, kskCount: members.length, unionNormal: allN.length, unionLatch: allL.length });
            try {
                broadcast({ type: 'aliases/union', mac, names: merged, normalPins: allN, latchPins: allL });
            }
            catch { }
        }
        catch { }
